...
(function() {
  ...
  const certId = getCertificationId();
  ...
  function downloadQuestion(questionId, type) {
    ...
    let content = `${certId.toUpperCase()} 기출문제 - 문제 ${questionId}\n`;
    content += `주제: ${topic}\n\n`;
    content += `문제:\n${questionText}\n\n`;
    content += `선택지:\n${options}\n\n`;
    ...
    if (type === 'question' || type === 'both') {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${certId.toUpperCase()}-문제-${questionId}.txt`;
      a.click();
  ...
  function downloadAllQuestions() {
    const cards = document.querySelectorAll('.question-card[data-converted]');
    let content = `${certId.toUpperCase()} 기출문제 모음\n`;
    content += '='.repeat(50) + '\n\n';
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${certId.toUpperCase()}-기출문제-전체.txt`;
    a.click();
    URL.revokeObjectURL(url);
  }
