...
THEME_RULES: list[tuple[str, list[str]]] = [
    ...
    ("finops", ["finops", "cost", "billing", "cost-optimization", "budget", "savings"]),
    (
        "blockchain",
        [
            "blockchain",
...
def detect_theme(tags: list[str], categories: list[str], title: str) -> str:
    ...
    haystack = " ".join(str(t).lower() for t in (tags + categories + [title]))
    scores: dict[str, int] = {}
...
def _add_unique_label(
    target: list[str], label: str, seen: set[str], limit: int
) -> None:
...
def build_visual_concepts(
    ...
    for fallback in THEME_DEFAULT_CONCEPTS.get(
        theme, THEME_DEFAULT_CONCEPTS["general"]
    ):
...
def build_excerpt_lines(
    ...
    clean = re.sub(
        r"\b(through|using|based|guide|complete|practical)\b",
        " ",
        clean,
        flags=re.IGNORECASE,
    )
    ...
    remaining_words = words[
        len(" ".join(lines + ([current] if current else [])).split()) :
    ]
    if current:
        lines.append(current)
    if remaining_words:
...
def build_signal_cards(
    concepts: list[str], excerpt: str, theme: str
    ...
    if isinstance(date_val, str):
        for fmt in ("%Y-%m-%d %H:%M:%S %z", "%Y-%m-%d"):
            try:
                return (
                    datetime.strptime(
                        date_val.strip()[:19], fmt[: len(date_val.strip()[:19])]
                    )
                    .strftime("%B %d, %Y")
                    .upper()
                )
...
def _variant_bands_decoration(
    ...
    specs = [
        (
            "470,186 1030,186 1150,286 590,286",
            p["accent1"],
            concepts[0],
            signal_cards[0][2],
        ),
        (
            "430,318 990,318 1110,418 550,418",
            p["accent2"],
            concepts[1],
            signal_cards[1][2],
        ),
        (
            "520,450 1080,450 1160,530 600,530",
            p["accent3"],
            concepts[2],
            signal_cards[2][2],
        ),
...
def generate_svg(
    ...
    decorations_svg = _common_decorations_v2(
        p, u, concept_labels, excerpt, theme, variant
    )
...
def process_post(post_path: Path, dry_run: bool, force: bool) -> tuple[bool, str]:
    ...
    if dry_run:
        return (
            True,
            f"DRY-RUN: {svg_path.name} ({old_kb:.1f}KB -> {new_kb:.1f}KB) theme={theme}",
        )

    svg_path.write_text(svg_content, encoding="utf-8")
    return (
        True,
        f"Upgraded: {svg_path.name} ({old_kb:.1f}KB -> {new_kb:.1f}KB) theme={theme}",
    )
...
def main():
    ...
    parser.add_argument(
        "--post", type=str, default=None, help="Process a specific post filename"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Show what would be done without writing"
    )
    parser.add_argument(
        "--force", action="store_true", help="Overwrite even if SVG is already >15KB"
    )
    ...
    if args.post:
        ...
        targets = sorted(
            p for p in POSTS_DIR.iterdir() if p.suffix == ".md" and p.is_file()
        )
        if not args.all:
    ...
    print(
        f"Processing {total} posts (before={args.before}, force={args.force}, dry_run={args.dry_run})"
    )
    print("-" * 70)
