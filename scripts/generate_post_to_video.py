...
def generate_audio(post_file: Path) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log_message(f"❌ 오디오 생성 실패 (종료 코드: {result.returncode})", "ERROR")
            return False
...
def find_thumbnail_image(post_metadata: dict, post_title: str = "", script: str = "", generate_if_missing: bool = False) -> Path:
    """
    썸네일 이미지를 찾거나 생성합니다.
    ...
    if generate_if_missing and post_title and script:
        generated_image = OUTPUT_DIR / f"{Path(post_metadata.get('title', 'default')).stem}_thumbnail.png"
        if generate_image_with_gemini_nano_banana(post_title, script, generated_image):
            if generated_image.exists():
                return generated_image
    
    # 썸네일이 없으면 에러
    raise FileNotFoundError("썸네일 이미지를 찾을 수 없습니다. assets/images 디렉토리에 이미지를 추가하거나 --generate-image 옵션을 사용하세요.")


def generate_video_with_ffmpeg(audio_path: Path, thumbnail_path: Path, output_path: Path) -> bool:
    """
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            log_message("   설치 방법: brew install ffmpeg (macOS) 또는 apt-get install ffmpeg (Linux)", "ERROR")
            return False
...
def generate_video_with_gemini_veo(audio_path: Path, thumbnail_path: Path, script: str, output_path: Path) -> bool:
    """
    ...
    GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "")
    if not GEMINI_API_KEY:
        log_message("⚠️ Gemini API 키가 없어 Gemini Veo 영상 생성을 건너뜁니다.", "WARNING")
        return False
...
def generate_video_with_remotion(post_file: Path, audio_path: Path) -> bool:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log_message(f"❌ Remotion 영상 생성 실패 (종료 코드: {result.returncode})", "ERROR")
            return False
...
def process_post(post_file: Path, method: str = "ffmpeg", skip_audio: bool = False, skip_video: bool = False, skip_image: bool = True, generate_image: bool = False) -> bool:
    """
    ...
    try:
        ...
        if not skip_image and generate_image:
            if script:
                ...
                if generate_image_with_gemini_nano_banana(title, script, generated_image):
                    if generated_image.exists():
        ...
        if not skip_video:
            ...
            if not thumbnail_path:
                try:
                    thumbnail_path = find_thumbnail_image(post.metadata, title, script, generate_image)
            ...
            if method == "ffmpeg":
                if not generate_video_with_ffmpeg(audio_path, thumbnail_path, video_path):
                    return False
                ...
                if not script:
                    log_message("⚠️ Gemini Veo는 대본이 필요합니다. 대본 파일을 확인하세요.", "WARNING")
                if not generate_video_with_gemini_veo(audio_path, thumbnail_path, script, video_path):
                    return False
            else:
...
예시:
  ...
  python3 scripts/generate_post_to_video.py --skip-audio
    ...
    parser.add_argument(
        "post_file",
        nargs="?",
        help="포스트 파일명 (선택사항, 없으면 최신 포스트 사용)"
    )
    parser.add_argument(
        "--method",
