...
def detect_language(code_lines: list[str]) -> str:
    ...
    for lang, patterns in LANGUAGE_PATTERNS:
        for pattern in patterns:
            if re.search(pattern, sample_text, re.MULTILINE | re.IGNORECASE if lang in ("sql",) else re.MULTILINE):
                scores[lang] += 1
    ...
    if best_lang == "bash":
        for competitor in ("python", "go", "javascript", "dockerfile", "nginx", "sql", "hcl"):
...
def process_file(filepath: Path, dry_run: bool) -> dict:
    ...
    while i < len(lines):
        ...
        if in_code_block:
            ...
            if closing and closing.group(2) == code_block_fence and closing.group(1) == code_block_indent:
                in_code_block = False
            ...
            if opening:
                ...
                if lang_tag == "":
                    ...
                    while j < len(lines):
                        ...
                        if closing and closing.group(2) == fence and closing.group(1) == indent:
                            break
                    ...
                    stats["changes"].append((i + 1, stripped, f"{indent}{fence}{detected_lang}", detected_lang))
...
def main():
    ...
    for filepath in md_files:
        ...
        if stats["fixed_blocks"] > 0:
            ...
            if args.verbose or args.dry_run:
                ...
                lang_summary = ", ".join(f"{lang}={cnt}" for lang, cnt in sorted(stats["languages"].items()))
    ...
    print(f"  Blocks {'would be ' if args.dry_run else ''}fixed:      {total_blocks_fixed}")
    print()
