    ...
    try:
        ...
        if dummy_issues:
            for line_num, match_text in dummy_issues:
                result['issues'].append(f"Line {line_num}: Possible dummy link - {match_text}")
...
예시:
  ...
  python3 scripts/fix_links_unified.py --fix _posts/2025-01-01-example.md
    ...
    parser.add_argument('--check', action='store_true', help='링크 확인만 수행 (수정하지 않음)')
    parser.add_argument('--fix', action='store_true', help='링크 수정 수행')
    parser.add_argument('--dry-run', action='store_true', help='수정 사항 미리보기 (실제 수정하지 않음)')
