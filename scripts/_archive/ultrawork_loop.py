#!/usr/bin/env python3
"""Run continuous ops checks and emit a Slack-ready summary."""
...
def run_link_checks(enabled: bool) -> CheckResult:
    if not enabled:
        return CheckResult("link-checks", True, "P2", "Skipped")
    ok, output = _run_command(["python3", "scripts/fix_links_unified.py", "--check"])
    summary = _summarize_output("fix_links_unified.py", output)
    priority = "P1" if not ok else "P2"
    return CheckResult("link-checks", ok, priority, summary)
...
def format_summary(results: List[CheckResult]) -> str:
    ...
    if any(result.priority == "P0" for result in failures):
        priority = "P0"
    elif any(result.priority == "P1" for result in failures):
        priority = "P1"

    header = f"Ultrawork Loop: {priority}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run continuous ops loop checks")
    parser.add_argument("--post-checks", action="store_true")
    parser.add_argument("--image-checks", action="store_true")
    parser.add_argument("--link-checks", action="store_true")
    ...
    link_checks = args.link_checks or os.getenv("RUN_LINK_CHECKS", "true") == "true"
    vercel_checks = (
