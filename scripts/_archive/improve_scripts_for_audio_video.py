...
def segment_for_remotion(
    text: str, target_duration_per_segment: float = 5.0
) -> List[dict]:
    """
...
- 오디오로 녹음했을 때 자연스럽고 듣기 좋아야 합니다."""
    ...
    try:
        ...
        safe_print(
            f"   ⚠️ Gemini API 오류: {safe_error}, 기본 개선만 적용", GEMINI_API_KEY
        )
        return original_text
...
def improve_script(script_path: Path) -> Tuple[str, List[dict]]:
    ...
    use_gemini = (
        os.getenv("USE_GEMINI_FOR_SCRIPT_IMPROVEMENT", "true").lower() == "true"
    )
    if use_gemini:
...
def save_improved_script(
    script_path: Path, improved_text: str, segments: List[dict], metadata: dict
):
    """개선된 대본 저장"""
    ...
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            for segment in segments:
                f.write(
                    f"[구간 {segment['index'] + 1}] 시작: {segment['startTime']:.2f}초, 길이: {segment['duration']:.2f}초\n"
                )
...
def main():
    ...
    # 대본 파일 찾기
    script_files = sorted(
        OUTPUT_DIR.glob("*_script.txt"), key=lambda p: p.stat().st_mtime, reverse=True
    )
    ...
    for script_file in script_files:
        ...
        try:
            ...
            if improved_text:
                ...
                output_path, json_path = save_improved_script(
                    script_file, improved_text, segments, metadata
                )
                if output_path:
                    results.append(
                        {
