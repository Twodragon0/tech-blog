    ...
    "2026-04-17-Tech_Security_Weekly_Digest_Botnet_Threat_AI_Malware": (
        ...
        (
            ...
            CardData(
                ...
                "Defender",
                "Zero-Day Active",
                "SonicWall brute force",
                "Multiple vendors face",
                "in-the-wild abuse",
                BLUE,
                "alert",
            ),
...
def _regenerate_derivatives(basename: str) -> None:
    ...
    cairosvg.svg2png(
        url=str(svg_path), output_width=1200, output_height=630, write_to=str(og_png)
    )
    ...
    cairosvg.svg2png(
        url=str(svg_path), output_width=525, output_height=275, write_to=str(card_png)
    )
    try:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Regenerate digest cover SVGs + derivatives."
    )
    parser.add_argument(
        "--phase",
        choices=("a", "b", "all"),
