...
def print_detailed(results):
    ...
    if needs_attention:
        ...
        for result in needs_attention:
            ...
            if result['main_sections_as_h3']:
                print(f"   ⚠️  주요 섹션이 H3로 되어 있음: {len(result['main_sections_as_h3'])}개")
    ...
    if low_h2:
        ...
        for result in low_h2[:10]:
            print(f"  - {result['file']}: H2 {result['h2_count']}개, H3 {result['h3_count']}개")
...
def print_verify(results):
    ...
    print("2. 목차 시스템은 렌더링된 HTML에서 <h2 id='...'> 태그를 찾아 목차를 생성합니다")
