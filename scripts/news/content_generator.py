...
def _select_excerpt_variant(
    ...
    'generic'을 반환한다.
    """
    signal = " ".join([title_keywords.lower()] + [t.lower() for t in (topics or [])])
...
def _compose_digest_post_title(
    ...
    series = (
        "기술 블로그 주간 다이제스트" if mode == "tech-blog" else "주간 보안 다이제스트"
    )
...
def generate_post_content(
    ...
    post_title = _compose_digest_post_title(
        date_str, news_items, total, mode="security"
    )
    ...
    _capped_title = (
        post_title[:80].rstrip(" ,.") if len(post_title) > 80 else post_title
    )
    ...
    yaml_description = sanitize_quotes_for_yaml(
        _build_clean_description(
            title_keywords, source_list, date_str, total, "security"
        )
    )
    yaml_image_alt = sanitize_quotes_for_yaml(
        _build_clean_image_alt(title_keywords, "security")
    )
...
'''
    ...
    rendered_items = (
        security_news + ai_news + cloud_news + devops_news + blockchain_news + tech_news
    )
...
def generate_tech_blog_content(
    ...
    post_title = _compose_digest_post_title(
        date_str, news_items, total, mode="tech-blog"
    )
    ...
    yaml_image_alt = sanitize_quotes_for_yaml(
        _build_clean_image_alt(title_keywords, "tech")
    )
...
def generate_news_section(
    ...
    if image or ko_summary:
        ...
        if ko_summary:
            ...
            card_summary = _sanitize_liquid_param(
...
"""
    ...
    if any(
        ...
        kw in text for kw in ["wallet", "지갑", "custody", "수탁", "key", "키 관리"]
    ):
        template += _pick_variant(
            item,
            [
