...
def build_block(score: int) -> str:
    ...
    priority = f"{tier} ({label})"
    row1 = (
        f"| 콘텐츠 밀도 | 점수 {score} 수준 "
        f"| 실무 의사결정 중심 문장 강화 | {priority} |"
    )
    lines = [
        "",
        "",
        ...
        "",
        "### 이번 라운드 개선 포인트",
        "- 핵심 위협과 비즈니스 영향의 연결 문장을 강화해"
        " 의사결정 맥락을 명확히 했습니다.",
        "- 운영팀이 바로 실행할 수 있도록"
        " 우선순위(P0/P1/P2)와 검증 포인트를 정리했습니다.",
        "- 후속 업데이트 시에는 실제 지표"
        "(MTTR, 패치 리드타임, 재발률)를 반영해 정량성을 높입니다.",
    ]
    return "\n".join(lines)
