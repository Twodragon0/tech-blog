...
def parse_post(filepath: Path) -> dict:
    ...
    cats = (
        re.findall(
            r"^\s*-\s+(\w+)",
            fm[fm.find("categories") : fm.find("categories") + 200],
            re.MULTILINE,
        )
        if "categories" in fm
        else []
...
def build_prompt(post: dict, filepath: Path = None) -> str:
    ...
    if any(
        w in kw_lower for w in ["cve", "vulnerability", "patch", "zero-day", "zero day"]
    ):
...
def main():
    ...
    parser.add_argument(
        "--engine", default="claude", choices=["claude", "codex", "gemini"]
    )
    parser.add_argument(
