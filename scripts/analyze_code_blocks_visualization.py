...
def print_report(results: Dict):
    ...
    print("🎨 VISUALIZATION CATEGORIES:")
    ...
    for lang, count in sorted(results["languages"].items(), key=lambda x: -x[1])[:15]:
        percentage = (
            (count / results["total_blocks"] * 100)
    ...
    print(
        f"  🔷 Mermaid Diagrams: {len(results['recommendations']['mermaid_diagram'])} blocks"
    )
    print("     → Convert to visual flowcharts, architectures, sequences")
    print(f"  🖼️  Code Images: {len(results['recommendations']['code_image'])} blocks")
    print("     → Convert to syntax-highlighted SVG images")
    print(
        f"  ✅ Existing Mermaid: {len(results['recommendations']['existing_mermaid'])} blocks"
    )
    print("     → Already visualized, keep as-is")
    print(f"  📝 Keep as Code: {len(results['recommendations']['keep_code'])} blocks")
    print("     → Small or simple blocks, keep as markdown")
    print()

    print("🎯 NEXT STEPS:")
    print("  1. Review Mermaid candidates and create diagram specifications")
    print("  2. Run generate_code_images.py to convert code blocks to images")
    print("  3. Update Mermaid.js version to v11 (currently v10.9.5)")
    print("  4. Test rendering performance with visualizations")
    print()

    # Show sample Mermaid candidates
    if results["recommendations"]["mermaid_diagram"]:
        print("📋 SAMPLE MERMAID CANDIDATES (First 5):")
        for i, rec in enumerate(results["recommendations"]["mermaid_diagram"][:5], 1):
            print(f"  {i}. {rec['file']}:{rec['line']}")
            print(
                f"     Language: {rec['language'] or '(none)'}, Length: {rec['length']} lines"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze code blocks for visualization strategy"
    )
    parser.add_argument("--output", "-o", help="Output JSON file path")
    parser.add_argument(
        "--posts-dir", default="_posts", help="Posts directory (default: _posts)"
    )
