...
def run_gemini_svg(
    ...
    prompt = (
        ...
        "\n[CRITICAL LAYOUT GRID — NO overlap allowed]\n"
        "- Canvas 1200x630, 3 horizontal bands: row1 y=0-210, row2 y=210-420, row3 y=420-630\n"
        "- Within each band, STRICT zone separation:\n"
        "  * LEFT TEXT ZONE: x=40 to x=520 (width 480) for all headlines/body/captions\n"
        "  * RIGHT ILLUSTRATION ZONE: x=600 to x=940 (width 340) for diagrams/graphics\n"
        "  * STAT BADGE ZONE: x=970 to x=1140 (width 170) for big numbers + labels\n"
        "- NO <text> may start at x>520 (except RIGHT/STAT zone internal labels)\n"
        "- NO <rect>/<path>/<g> illustration may extend into x<600 region\n"
        "- Headline font-size 24-28px (max 28), body 13-15px, caption 10-12px\n"
        "- Headlines must fit within LEFT TEXT ZONE (480px). For text >32 chars, "
        "split into two lines OR add textLength='480' lengthAdjust='spacingAndGlyphs'\n"
        "- Keep 80px gutter (x=520-600) empty between text and illustration columns\n"
        "\n[OUTPUT]\n"
...
def run_codex_svg(
    ...
    prompt = (
        ...
        "\n[CRITICAL LAYOUT GRID — overlap 절대 금지]\n"
        "- 캔버스 1200x630, 3 horizontal bands: row1 y=0-210, row2 y=210-420, row3 y=420-630\n"
        "- 각 band 내부 ZONES (엄격히 분리):\n"
        "  * LEFT TEXT ZONE: x=40 ~ x=520 (width 480) — 모든 헤드라인/본문/캡션 텍스트\n"
        "  * RIGHT ILLUSTRATION ZONE: x=600 ~ x=940 (width 340) — 다이어그램/그래픽/아이콘\n"
        "  * STAT BADGE ZONE: x=970 ~ x=1140 (width 170) — 큰 숫자 + 라벨\n"
        "- 어떠한 <text>도 x>520 위치에 시작하면 안 됨 (단, RIGHT/STAT zone 내부 라벨 제외)\n"
        "- 어떠한 <rect>/<path>/<g> illustration이 x<600 영역을 침범하면 안 됨\n"
        "- 헤드라인 font-size 24-28px (28을 넘기지 말 것), 본문 13-15px, 캡션 10-12px\n"
        "- 헤드라인 텍스트 length는 LEFT TEXT ZONE width(480px) 안에 들어가야 함. "
        "32자 이상이면 두 줄로 split하거나, textLength='480' lengthAdjust='spacingAndGlyphs' 추가\n"
        "- 좌측 텍스트 column과 우측 일러스트 column 사이 80px gutter (x=520-600) 비워둘 것\n"
        "\n[OUTPUT]\n"
