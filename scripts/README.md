...
스크립트 작업 시 Opus 4.6를 효과적으로 활용하기 위한 팁:
...
```bash
# 스크립트를 실행하기 전에:
# "모든 포스트의 링크를 검증하고, 깨진 링크를 자동으로 수정해줘. 
#  변경 사항은 먼저 보고하고, 확인 후 적용해줘."
```

**❌ 나쁜 예시**:
```bash
# "링크 확인해줘. 그리고 깨진 거 있으면 고쳐줘. 
#  그리고 잊지 말고 보고도 해줘."
```
...
scripts/ 디렉토리에 Python 스크립트들이 있고,
_posts/ 디렉토리에 마크다운 포스트가 있습니다.
이 구조를 이해한 후 새로운 검증 스크립트를 작성해줘."
```
