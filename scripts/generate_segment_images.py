...
def generate_image_prompt_for_segment(
    segment_text: str, keywords: List[str], post_title: str = ""
) -> str:
            ...
            if response.status_code == 200:
                ...
                if "candidates" in result and len(result["candidates"]) > 0:
                    ...
                    if "text" in candidate.get("content", {}).get("parts", [{}])[0]:
                        ...
                        log_message(
                            f"⚠️ Gemini API가 텍스트 응답을 반환했습니다. 이미지 생성 프롬프트로 사용할 수 있습니다.",
                            "WARNING",
                        )
                        ...
                        if _validate_masked_text(safe_prompt) and _validate_masked_text(
                            safe_text_response
                        ):
                    ...
                    log_message(
                        f"⚠️ Gemini API 응답에 이미지 데이터가 없습니다.", "WARNING"
                    )
                    log_message(
                ...
                log_message(
                    f"❌ 이미지 생성 실패: HTTP {response.status_code}", "ERROR"
                )
...
def generate_segment_images(
    segments_json_path: Path, post_title: str = ""
) -> Dict[str, str]:
    """세그먼트별 이미지 생성"""
    if not segments_json_path.exists():
        log_message(
            f"❌ 세그먼트 JSON 파일을 찾을 수 없습니다: {segments_json_path}", "ERROR"
        )
        return {}
    ...
    try:
        ...
        for i, segment in enumerate(segments):
            ...
            safe_title = (
                post_title.replace(" ", "_").replace("/", "_")[:50]
                if post_title
                else "segment"
            )
            ...
            if image_path.exists():
                log_message(
                    f"⏭️ 세그먼트 {segment_index} 이미지 이미 존재: {image_filename}"
                )
                image_mapping[f"segment_{segment_index}"] = image_filename
                continue

            # 이미지 생성 프롬프트 생성
            prompt = generate_image_prompt_for_segment(
                segment_text, keywords, post_title
            )
            ...
            if generate_image_with_gemini(prompt, image_path):
                image_mapping[f"segment_{segment_index}"] = image_filename
                log_message(
                    f"✅ 세그먼트 {segment_index} 이미지 생성 완료: {image_filename}",
                    "SUCCESS",
                )
            else:
...
def main():
    ...
    if len(sys.argv) < 2:
        log_message(
            "사용법: python generate_segment_images.py <post_basename>", "ERROR"
        )
        log_message(
            "예시: python generate_segment_images.py 2025-05-23-클라우드_시큐리티_과정_7기_-_6주차_Cloudflare_및_github_보안",
            "ERROR",
        )
        sys.exit(1)
