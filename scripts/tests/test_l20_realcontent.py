...
class TestMultiWordVendorPromote:
    ...
    def test_arch_linux_promoted(self):
        # FM5: 06-13/06-15 real highlights. "linux" ∈ _GENERIC_TRAILING strips to a
        # lone "Arch"; "Arch Linux" is a real distro entity present in the title.
        assert (
            build_lead_headline(
    ...
    def test_malware_plus_linux_not_promoted(self):
        ...
        # the malware/vuln (the real subject); "Linux" is just the platform, so the
        # lone lead is CORRECT and must NOT promote to a fabricated "<X> Linux".
        assert (
            build_lead_headline(
...
class TestMultiWordVendorVetting:
    ...
    Seed is EXACTLY the curated live-firing pairs (FM4: red hat, amazon bedrock;
    FM5: arch linux) — each a real entity with an exercising live cover, no
    unexercised forward-looking entries. The corpus invariant is PROPERTY-based,
    not a fixed count: daily
    ...
    def test_seed_is_exactly_the_curated_pairs(self):
        ...
        # FM4 shipped {red hat, amazon bedrock}; FM5 added {arch linux} WITH its
        # exercising titles. Every pair here is a real entity with a live cover.
