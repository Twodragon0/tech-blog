...
def test_returns_none_when_no_article_text():
    out = summary_expander.expand_summary(
        {"title": "t", "url": "u"}, "", gemini=lambda p, timeout=35: "x"
    )
    assert out is None
...
def test_honesty_rejects_unsupported_cve():
    assert (
        summary_expander.is_source_grounded(
            "CVE-2026-9999 관련 위협", "article without that cve"
        )
        is False
    )
...
def test_honesty_rejects_unsupported_number():
    assert (
        summary_expander.is_source_grounded(
            "총 622개의 취약점이 발견", "article mentions a few bugs, no counts"
        )
        is False
    )
...
def test_honesty_accepts_grounded_text():
    ...
    assert (
        summary_expander.is_source_grounded("CVE-2026-1234 는 3 개 제품에 영향", src)
        is True
    )
...
def test_honesty_rejects_substring_number_false_ground():
    ...
    assert (
        summary_expander.is_source_grounded(
            "영향받은 시스템은 42대", "the incident involved 1425 hosts"
        )
        is False
    )
...
def test_honesty_accepts_exact_number_token():
    ...
    assert (
        summary_expander.is_source_grounded("총 1,000건", "reported 1000 cases") is True
    )
...
def test_returns_none_when_llm_raises():
    ...
    out = summary_expander.expand_summary(
        {"title": "t", "url": "u"}, "some real article text " * 30, gemini=boom
    )
    assert out is None
