...
| `docs/scripts/` | Script-specific docs |
...
| 작업 유형 | 모델 | 이유 |
|-----------|------|------|
| 콘텐츠/코드 생성 | Opus 4.6 ⭐ | 고품질 출력 필요 |
| 코드 작성/리팩토링 | Opus 4.6 ⭐ | 복잡한 로직, 모범 사례 |
| 검증/분석 | Sonnet 4.6💰 | 규칙 기반, 비용 효율적 |
| 읽기 전용 탐색 | Sonnet 4.6💰 | 분석만, 생성 없음 |
| 보안 감사 | Sonnet 4.6💰 | 패턴 매칭, 비용 효율적 |

### Ralph Loop Commands

| Command | Model | Description | Completion Promise |
|---------|-------|-------------|-------------------|
| `/improve-posts` | Opus 4.6 ⭐ | RSS 수집 및 포스트 개선 | `POSTS_IMPROVED` |
| `/collect-news` | Sonnet 4.6💰 | RSS 뉴스 수집 (비용 최적화) | `NEWS_COLLECTED` |
| `/validate-posts` | Sonnet 4.6💰 | 포스트 품질 검증 (비용 최적화) | `POSTS_VALIDATED` |
| `/generate-images` | Opus 4.6 ⭐ | 이미지 생성 | `IMAGES_GENERATED` |
| `/security-audit` | Sonnet 4.6💰 | 보안 감사 (비용 최적화) | `SECURITY_AUDIT_COMPLETE` |
| `/write-code` | Opus 4.6 ⭐ | 코드 작성 | `CODE_WRITTEN` |
| `/refactor` | Opus 4.6 ⭐ | 코드 리팩토링 | `CODE_REFACTORED` |
| `/fix-bugs` | Opus 4.6 ⭐ | 버그 및 보안 이슈 수정 | `BUGS_FIXED` |
| `/cost-optimize` | Sonnet 4.6💰 | API 사용 최적화 분석 | `COST_OPTIMIZED` |
...
4. **배치 처리**: 여러 작업을 그룹화
5. **모델 선택**: 검증은 Sonnet 4.6, 생성은 Opus 4.6
...
6. **Cost Optimization**: 
   ...
   - Smart model selection (Opus 4.6 for generation, Sonnet 4.6for validation)
...
|-----------|-------|------|
| `blog-lead.md` | opus | 프로젝트 리드, 콘텐츠 전략, 작업 분배 |
| `architect.md` | opus | Jekyll 구조 설계, 스크립트 아키텍처 |
| `post-researcher.md` | sonnet | 기술 블로그 주제 리서치, 트렌드 분석 |
| `post-quality-reviewer.md` | haiku | 포스트 품질 검증, 마크다운 린팅 |
| `post-validator.md` | haiku | 포스트 유효성 검사 (프론트매터, 링크, 이미지) |
| `security-auditor.md` | sonnet | 보안 감사, CSP/HSTS, 의존성 검사 |
| `seo-optimizer.md` | haiku | SEO 메타데이터, Open Graph, JSON-LD |
| `performance-analyst.md` | haiku | Core Web Vitals, 캐싱, 로딩 성능 |
| `test-engineer.md` | sonnet | 스크립트 테스트, 템플릿 분기 테스트, CI 검증 |
