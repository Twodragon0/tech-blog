...
excerpt: "정보 기관 주장 이후 제재 대상 Grinex 거래소가 1,374만 · 크로스 테넌트 헬프데스크 사칭에서 데이터 유출까지: 인간 운영 침입이 부각된 2026년 04월 19일 보안 다이제스트 — 15건의 이슈와 실행 가능한 대응 액션을 정리합니다. 변경 통제와 모니터링 적용 시점, 사후 회고에 활용할 IoC 정리표를 포함합니다."
...
| 분류 | 핵심 이슈 | 심각도 | 출처 |
|------|----------|--------|------|
| 🔒 **Security** | 정보 기관 주장 이후 제재 대상 Grinex 거래소가 1,374만 달러 해킹으로 폐쇄 | 🟠 High | The Hacker News |
| 🔒 **Security** | 크로스 테넌트 헬프데스크 사칭에서 데이터 유출까지: 인간 운영 침입 플레이북 | 🟠 High | Microsoft Security B |
| 🔒 **Security** | Mirai 변종 Nexcorium, CVE-2024-3721 취약점으로 TBK DVR 장악해 DDoS 봇넷 구축 | 🟠 High | The Hacker News |
| ⛓️ **Blockchain** | 워런, SEC의 앳킨스가 집행 데이터로 의회를 오도했을 가능성 주장 | 🟠 High | Cointelegraph |
| ⛓️ **Blockchain** | Solana 선물 미결제약정이 이번 주 20% 증가: SOL 100달러 다음 목표일까? | 🟡 Medium | Cointelegraph |
| ⛓️ **Blockchain** | 이란, BTC를 전략적 자산으로 간주하지만 USDt가 여전히 유류 통행료 지배: BPI | 🟡 Medium | Cointelegraph |
| 💻 **Tech** | World Monitor - 실시간 글로벌 인텔리전스 대시보드 | 🟡 Medium | Tech World Monitor |
| 💻 **Tech** | Show GN: Paperclip + Gastown에서 영감을 받은, “영구기관” 컨셉의 오픈소스 AI 에이전트 프레임워크 | 🟡 Medium | GeekNews (긱뉴스) |
| 💻 **Tech** | 분리된 구간 집합 위에서 계산하는 계산기 | 🟡 Medium | GeekNews (긱뉴스) |
...
## 1. 기술적 배경 및 위협 분석
이 공격은 Microsoft Teams의 외부 협업 기능을 악용한 다단계 위협입니다. 공격자는 외부 테넌트에서 IT 헬프데스크를 사칭하여 Teams 메시지를 보내고, 사용자를 속여 **Quick Assist**로 원격 접속 권한을 획득합니다. 내부 진입 후에는 **Living off the Land(LotL)** 전술을 활용해 PowerShell로 정찰하고, 신뢰된 서명 앱에 대한 DLL 사이드로딩으로 상주한 뒤 HTTPS(443)로 C2에 비콘합니다. 이어서 **WinRM** 자격증명 기반 측면 이동으로 도메인 컨트롤러 등 고가치 시스템에 접근하고, `msiexec`로 상용 원격관리 도구(Level RMM)를 심어 대체 제어 채널을 확보한 뒤 **Rclone**으로 문서를 외부 클라우드로 유출합니다. 이는 정상적인 IT 지원 활동으로 위장하기 때문에 전통적인 시그니처 기반 탐지로는 발견하기 어렵습니다. Microsoft Defender는 Teams, 엔드포인트, ID 원격 분석을 연계하여 이러한 비정상적인 행위 패턴을 탐지합니다.

```mermaid
sequenceDiagram
    actor Attacker as 공격자(외부 테넌트)
    participant Teams as Microsoft Teams
    actor User as 사용자
    participant QA as Quick Assist
    participant Host as 내부 호스트
    participant C2 as 공격자 C2 서버
    participant Storage as 외부 클라우드 저장소
    participant Defender as Microsoft Defender

    Attacker->>Teams: IT 헬프데스크 사칭 메시지 전송
    Teams->>User: 외부 발신자 경고 표시
    User->>Attacker: 경고 무시하고 응답
    Attacker->>User: Quick Assist 원격 지원 요청
    User->>QA: 원격 접근 권한 승인
    QA->>Host: 대화형 세션 획득
    Host->>Host: PowerShell 정찰 + DLL 사이드로딩 상주
    Host->>C2: HTTPS 비콘 (TCP 443)
    Host->>Host: WinRM 자격증명 기반 측면 이동
    Host->>Storage: Rclone으로 문서 유출
    Defender-->>Host: 다단계 이상행위 탐지 및 세션 차단
```
...
## 1. 기술적 배경 및 위협 분석
이 공격은 **Mirai 봇넷의 변종인 Nexcorium**이 중간 심각도(CVSS 6.3)의 **명령어 삽입 취약점(CVE-2024-3721)**을 통해 TBK DVR 장치를 탈취하는 사례입니다. 이 취약점은 **인증이 필요 없는(unauthenticated)** 명령어 삽입으로, 공격자는 `/device.rsp?opt=sys&cmd=___S_O_S_T_R_E_A_MAX___` 엔드포인트에 조작된 HTTP 요청을 보내 인증 절차 없이 곧바로 시스템 명령을 삽입하고 봇넷 에이전트를 배포합니다. 특히 주목할 점은 **EoL(수명 주기 종료) TP-Link 라우터**도 함께 표적으로 삼고 있다는 것입니다. 이는 공격자가 보안 패치가 중단된 취약한 IoT 장치를 체계적으로 탐색 및 악용하여 대규모 DDoS 공격 인프라를 구축하고 있음을 보여줍니다. Mirai 변종은 지속적으로 새로운 취약점을 탐색하며, 기존 시그니처 기반 탐지 회피 기법을 진화시키고 있습니다.

```mermaid
sequenceDiagram
    participant Scanner as 공격자 스캐너
    participant DVR as TBK DVR (미인증)
    participant Downloader as 다운로더 스크립트
    participant Botnet as Nexcorium 봇넷 C2
    participant Victim as DDoS 표적

    Scanner->>DVR: /device.rsp?opt=sys&cmd=...SOSTREAMAX... 요청 (인증 불필요)
    DVR->>DVR: mdb/mdc 인자 통한 명령어 삽입 실행
    DVR->>Downloader: 다운로더 스크립트 획득
    Downloader->>DVR: 아키텍처별 봇넷 페이로드 설치
    DVR->>DVR: 원본 바이너리 삭제 (분석 회피)
    DVR->>DVR: crontab/systemd로 지속성 확보
    DVR->>Botnet: C2 등록 및 명령 대기
    Botnet->>DVR: DDoS 공격 명령 하달
    DVR->>Victim: UDP/TCP/SMTP DDoS 트래픽 발생
```
