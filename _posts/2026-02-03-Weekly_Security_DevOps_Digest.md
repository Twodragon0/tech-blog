...
summary_card:
  ...
  tags:
    ...
    - "2026"
  highlights:
    - { source: "AI(Moltbot) 연쇄 보안 사건", title: "CVE-2026-25253 원클릭 RCE(CVSS 8.8), ClawHavoc 캠페인 335개 Atomic Stealer 배포, Moltbook 자격증명 대량 유출(Wiz), 가짜 VS Code 확장 ScreenConnect RAT, Shodan 대규모 인스턴스 노출" }
    - { source: "MDM 앱 비활성화 실무 가이드", title: "Jamf Pro Configuration Profile 기반 앱 제한, Microsoft Intune App Protection Policy, MDM 선택 의사결정 플로차트 제공" }
    - { source: "금주 뉴스 하이라이트", title: "Microsoft NTLM 3단계 폐지, Snowflake-OpenAI 2억달러 파트너십, Kubernetes 1.33 보안 강화, SBOM 컴플라이언스 동향" }
---
...
- **긴급 대응 필요**: 원클릭 RCE(CVSS 8.8) — 인증 토큰 탈취 기반 원격 코드 실행, 즉시 패치 필요, ClawHub 악성 스킬 341개 중 335개 macOS Atomic Stealer(AMOS) 배포 확인 등 Critical 등급 위협 2건이 확인되었습니다.
- **주요 모니터링 대상**: AI 소셜네트워크 Moltbook에서 수백만 건 자격증명 인증 없이 노출, Clawdbot 사칭 확장이 ScreenConnect RAT 설치 — 공급망 위협 등 High 등급 위협 2건에 대한 탐지 강화가 필요합니다.
...
- 조직 내 AI 설치 현황 인벤토리 확인 (섹션 1.7의 Jamf EA 스크립트 활용)
...
### 1.7 Jamf Extension Attribute: AI/Moltbot 설치를 어떻게 탐지하는가?
...
| AI 에이전트 제어 | EA 스크립트 + 프로파일 (섹션 1.7) | Compliance Policy + App Block |
...
- [ ] **원클릭 RCE(CVSS 8.8) — 인증 토큰 탈취 기반 원격 코드 실행, 즉시 패치 필요** 관련 긴급 패치 및 영향도 확인
- [ ] **ClawHub 악성 스킬 341개 중 335개 macOS Atomic Stealer(AMOS) 배포 확인** 관련 긴급 패치 및 영향도 확인

### P1 (7일 내)

- [ ] **AI 소셜네트워크 Moltbook에서 수백만 건 자격증명 인증 없이 노출** 관련 보안 검토 및 모니터링
- [ ] **Clawdbot 사칭 확장이 ScreenConnect RAT 설치 — 공급망 위협** 관련 보안 검토 및 모니터링

### P2 (30일 내)

- [ ] **2억 달러 규모 전략적 파트너십 체결 — 엔터프라이즈 AI 데이터 플랫폼 통합** 관련 AI 보안 정책 검토
...
| 제목 | URL |
|------|-----|
| Weekly Security Threat Intelligence Digest (Feb 2) | [주간 보안 위협 인텔리전스 다이제스트 (Feb 2)](/2026-02-02-Weekly_Security_Threat_Intelligence_Digest) |
| Weekly Tech & AI & Blockchain Digest (Feb 2) | [주간 기술·AI·블록체인 다이제스트 (Feb 2)](/2026-02-02-Weekly_Tech_AI_Blockchain_Digest) |
