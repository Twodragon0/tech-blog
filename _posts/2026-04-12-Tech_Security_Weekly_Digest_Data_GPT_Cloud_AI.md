...
오늘의 두 핵심은 **Citizen Lab 의 Webloc(Cobwebs Technologies) 분석 — 광고 데이터로 5억 대 기기 추적** 과 **CPU-Z·HWMonitor 변조 다운로드를 통한 STX RAT 유포**다. 둘은 같은 결로 만난다 — **개발자·운영자가 "출처를 의심하지 않던" 합법 채널(광고 SDK, 공식 사이트로 보이는 다운로드)** 이 1차 침투 경로로 굳어진다는 점이다. Webloc 은 사내 모바일 앱이 광고 변현을 위해 통합한 SDK 가 실시간 위치 데이터를 모조리 광고 거래소에 흘려보내고 있다는 사실을 보여주고, CPU-Z 케이스는 IT/보안팀이 평소 사용하던 진단 도구가 멀웨어 dropper 로 둔갑할 수 있음을 보여준다.
