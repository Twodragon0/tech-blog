...
- USIM
- Security-Incident
keywords: [SKT, MFA, USIM, Security-Incident]
...
description: "SK텔레콤 USIM 정보 유출 사태 완벽 대응 가이드. USIM/eSIM 즉시 교체 방법, IMEI 확인, MFA 활성화, SIM 스와핑 공격 위험성과 방어 전략, 통신사 보안 서비스 가입, 금융 계좌 보호 설정까지 개인과 기업이 취해야 할 실무 중심 보안 조치를 단계별로 정리합니다."
...
summary_card:
  ...
  tags:
    ...
    - "Security-Incident"
  highlights:
    - { source: "SK텔레콤 USIM 정보 유출 사태 대응", title: "즉시 조치(USIM/eSIM 교체, IMEI 확인, MFA 활성화), 통신사 보안 서비스 가입(USIM 보호 서비스, 번호 도용 차단), 금융 보안 강화(계좌 이체 한도 조정, 생체 인증 설정, 이상 거래 알림)" }
    - { source: "SIM 스와핑/복제 위험성", title: "공격 메커니즘(통신사 직원 사칭, 사회공학적 기법, 내부자 협조), 피해 사례(계좌 이체, 금융 앱 접근, 2FA 우회), OTP/MFA의 중요성(SMS OTP 취약점, TOTP 앱 권장, 하드웨어 보안 키)" }
    - { source: "IMEI 확인 및 USIM/eSIM 교체", title: "IMEI 확인 방법(아이폰 *#06#, 안드로이드 설정→휴대전화 정보), 안전한 USIM/eSIM 교체 절차(통신사 방문, 본인 확인 강화, 즉시 활성화), eSIM 전환 장점(물리적 복제 불가, 원격 관리 가능)" }
    - { source: "2025년 업데이트", title: "통신사 보안 강화 조치(무료 USIM 보호 서비스, eSIM 전환 촉진, 이중 인증 강화, 실시간 이상 탐지 시스템, IMEI 변경 알림), 개인 보안 강화 권고사항(MFA 전면 적용, 통신사 보안 서비스 가입, 정기 IMEI 확인)" }
    - { source: "기업 보안 시사점", title: "공급망 보안(외부 파트너 보안 수준 점검), Zero Trust 아키텍처 적용, 보안 사고 대응 계획 수립, AI 기반 피싱 공격 대응 전략" }
---
...
| 항목 | 평가 | 설명 |
|------|------|------|
| 위험도 | 🔴 매우 높음 | USIM 정보 유출로 SIM 스와핑 공격 가능 |
...
- USIM 일련번호 (ICCID - 집적 회로 카드 식별자)
- IMSI (국제 이동 가입자 식별번호)
- Ki 값 (인증 키, 일부 사례)
- 가입자 전화번호
- 계약자 성명 및 생년월일
- 기기 IMEI (국제 이동 단말기 식별번호)
...
| ATT&CK 전술 | 기법 ID | 기법 이름 | SIM 스와핑 연관성 |
|---------------|---------|----------|-------------------|
| 정찰 (Reconnaissance) | T1589 | 피해자 신원 정보 수집 | 다크웹에서 유출 정보 수집 |
| 자원 개발 (Resource Development) | T1586 | 계정 침해 | 통신사 직원 계정 침해 |
| 초기 접근 (Initial Access) | T1078 | 유효 계정 활용 | 유출된 인증 정보로 통신사 시스템 접근 |
| 자격증명 접근 (Credential Access) | T1110 | 무차별 대입 | 고객 계정 무차별 대입 공격 |
| 자격증명 접근 (Credential Access) | T1556 | 인증 프로세스 변조 | SIM 교체로 2FA 우회 |
| 수집 (Collection) | T1114 | 이메일/메시지 수집 | SMS/이메일 수신 가로채기 |
| 영향 (Impact) | T1565 | 데이터 조작 | 금융 거래 정보 조작 |
...
| 1Password | 전 플랫폼 | 비밀번호 관리자 통합 | ✅ 클라우드 |
| Bitwarden | 전 플랫폼 | 오픈소스, 자체 호스팅 가능 | ✅ 클라우드 |

Google Authenticator 설정 예시:

설정 앱 → 보안 → 2단계 인증 → TOTP 앱 추가 → QR코드 스캔

#### 4.2.2 하드웨어 보안 키 (FIDO2)
...
IMEI 15자리 = TAC(6자리) + FAC(2자리) + SNR(6자리) + CD(1자리)

#### 4.3.2 통신사 IMEI 변경 알림 서비스

SKT IMEI 변경 알림 설정:

T월드 앱 → 보안 설정 → IMEI 변경 알림 활성화

### 4.4 금융 보안 강화

금융감독원 긴급 지침:

- SMS OTP 거래는 추가 인증 단계 적용
- 이상 거래 탐지 시스템 강화
...
| 유형 | 제목 | 제공 |
|------|------|------|
| 온라인 코스 | 사이버보안 기초 (Cybersecurity Essentials) | Cisco Networking Academy |
| 온라인 코스 | 사이버보안 입문 (Introduction to Cybersecurity) | Coursera / NYU |
| 웨비나 | SIM 스와핑 공격 예방 | SANS Institute |
| 백서 | 다중 인증 가이드 (Multi-Factor Authentication Guide) | Microsoft Security |
| 블로그 | SIM 스와핑의 작동 원리 | Krebs on Security |
...
- SMS 인증 의존도를 제거하고 더 강력한 인증 수단으로 전환해야 합니다
- 지속적인 모니터링과 위협 헌팅을 통해 능동적으로 위협을 탐지해야 합니다

### 마지막 당부

보안은 일회성 조치가 아니라 지속적인 프로세스입니다. 이 가이드의 체크리스트를 단계별로 실행하고, 정기적으로 보안 상태를 점검하며, 새로운 위협에 대한 정보를 계속 업데이트해야 합니다.
