...
if command -v docker &> /dev/null; then
    echo "[*] Docker 컨테이너 확인..."
    docker ps --filter "ancestor=n8nio/n8n" --format "{% raw %}{{.Names}}: {{.Image}}{% endraw %}" 2>/dev/null
...
> 
> ```yaml
> # sigma/rules/application/n8n_sandbox_escape_attempt.yml [truncated]
> ```
...
> 
> ```yaml
> # D-Link dnscfg.cgi 커맨드 인젝션 탐지 룰 [truncated]
> ```
...
> 
> ```yaml
> # cilium-ai-agent-policy.yaml [truncated]
> ```
...
> 
> ```yaml
> # tetragon-ai-agent-tracing.yaml [truncated]
> ```
...
> 
> ```yaml
> # falco-ai-agent-rules.yaml [truncated]
> ```
...
> 
> ```yaml
> # 데이터 주권 요구사항 평가 체크리스트 [truncated]
> ```
...
> 
> ```yaml
> # nhi-security-policies.yaml [truncated]
> ```
