...
2026년 1월에 공개된 [CVE-2026-25253](https://nvd.nist.gov/)은 OpenClaw의 스킬(Skill) 시스템에 존재하는 **원격 코드 실행(RCE) 취약점**입니다. CVSS 점수 8.8로 **High Severity**에 해당합니다.
...
```python
# OpenClaw 스킬 로더 - 취약한 코드 (의사 코드)
# 경고: 이 코드는 취약점 설명 목적이며 실제 코드가 아닙니다
class SkillLoader:
    def install_skill(self, skill_url):
        # 문제 1: URL 검증 없음
        skill_package = self.download(skill_url)

        # 문제 2: 코드 서명 검증 없음
        # 문제 3: 샌드박스 없이 직접 실행
        exec(skill_package.on_install)  # RCE 발생 지점

        # 문제 4: 권한 분리 없음 - 호스트 시스템 전체 접근
        self.register_skill(skill_package)
```
...
```javascript
// WebSocket Hijacking 공격 시나리오 (교육 목적)
// 공격자가 호스팅하는 악성 웹페이지
const ws = new WebSocket('ws://victim-openclaw:8080/ws');
ws.onmessage = (event) => {
    // 모든 메시지 도청
...
ws.send(JSON.stringify({
    type: 'command',
    payload: 'env | grep API_KEY'  // 환경변수 탈취
}));
```
...
### Q1: OpenClaw가 Claude Code보다 저렴한가?

**A: 아니오.** 표면적으로 OpenClaw는 오픈소스(무료)이지만, 실제 운영 비용은 훨씬 높습니다.

- Claude Code Pro: **$20/월** (대부분의 개인 개발자에게 충분)
- Claude Code Max: **$100-200/월** (고사용량)
- OpenClaw + Opus 4.5 API: **$300-750/월** (Context Caching 미적용으로 토큰 2-3배 소비)
- OpenClaw + 인프라: **추가 $100-500/월** (서버, GPU, 유지보수)

Claude Code는 Prompt Caching, 최적화된 컨텍스트 관리 등으로 동일 작업에 대해 **더 적은 토큰을 소비**합니다.

### Q2: OpenClaw가 더 안전한가?

**A: 절대 아니오.** OpenClaw는 현재 AI 개발 도구 중 가장 심각한 보안 문제를 안고 있습니다:

- **CVE-2026-25253** (CVSS 8.8): One-click RCE 취약점
- **400+ 악성 스킬**: 마켓플레이스에 악성 코드 대량 유통
- **WebSocket Hijacking**: 통신 도청 및 명령 주입 가능
- **보안 인증 ZERO**: SOC 2, ISO 27001 등 어떤 인증도 없음

Claude Code는 **SOC 2 Type II 인증**, **GDPR/CCPA 준수**, **AES-256 암호화**를 갖추고 있습니다.

### Q3: OpenClaw가 오픈소스라서 더 투명하지 않은가?

**A: 이론적으로는 맞지만, 실무에서는 다릅니다.**

오픈소스의 투명성은 **커뮤니티가 실제로 코드를 리뷰하고 감사할 때만** 의미가 있습니다. CVE-2026-25253이 발견되기까지 상당 기간 존재했다는 사실은, 오픈소스라고 해서 자동으로 안전하지 않음을 보여줍니다. 반면 Claude Code는 전담 보안팀이 지속적으로 코드를 감사하고, SOC 2 Type II 인증을 통해 독립적 검증을 받고 있습니다.

### Q4: OpenClaw와 Claude Code를 함께 사용할 수 있는가?

**A: 기술적으로는 가능하지만, 권장하지 않습니다.**

OpenClaw는 Claude API를 백엔드로 사용할 수 있습니다. 그러나 이 경우:

- Claude Code의 **Prompt Caching 최적화를 활용하지 못함**
- 중간에 OpenClaw 레이어가 추가되어 **보안 공격 표면 증가**
- **비용이 2-3배 증가** (최적화 없이 API 직접 호출)
- OpenClaw의 보안 취약점이 Claude API 키를 위험에 노출

**코딩 작업에는 Claude Code를, 메시징 봇 구축에는 보안이 강화된 OpenClaw를** 각각의 목적에 맞게 분리 사용하는 것이 올바른 접근입니다.

### Q5: 로컬 LLM을 사용하면 OpenClaw가 더 안전하지 않은가?

**A: 부분적으로 맞습니다.** 로컬 LLM(Ollama, llama.cpp 등)을 사용하면 데이터가 외부로 전송되지 않습니다. 그러나:

- CVE-2026-25253 (RCE 취약점)은 **LLM 백엔드와 무관하게** 존재
- 악성 스킬 문제는 **로컬 모델 사용 여부와 무관**
- WebSocket Hijacking은 **OpenClaw 자체의 취약점**
- 로컬 모델의 코딩 능력은 Claude Opus 4.5 대비 **현저히 낮음**

### Q6: Claude Code Enterprise 도입에 얼마나 걸리는가?

**A: 일반적으로 1-4주입니다.**

| 단계 | 소요 시간 | 활동 |
|------|-----------|------|
| **평가** | 1-3일 | Pro 플랜으로 기능 평가 |
| **PoC** | 1-2주 | 실제 프로젝트에 적용 테스트 |
| **보안 검토** | 1주 | 보안팀 검토 (SOC 2 문서 제공) |
| **팀 롤아웃** | 1주 | Team/Enterprise 플랜 전환, 온보딩 |

### Q7: AI 코딩 어시스턴트가 개발자를 대체할 수 있는가?

**A: 현재로서는 아니오.** AI 코딩 어시스턴트는 **개발자의 생산성을 증폭**시키는 도구이지, 대체하는 것이 아닙니다:

- SWE-bench Verified 80.9%는 인상적이지만, **19.1%는 여전히 해결 불가**
- 아키텍처 설계, 비즈니스 로직 이해, 팀 의사소통은 **인간의 영역**
- 23.7% 보안 취약점 증가는 **인간의 리뷰가 필수**임을 보여줌
- 75%의 성공적인 개발자가 **모든 AI 코드를 수동 리뷰**

---

## 10. Quick Start 가이드

### 10.1 Claude Code: 5분 설치 가이드
...
### 10.2 OpenClaw: 보안 주의사항 포함 설치 가이드
...
# Step 4: 보안 강화 설정 (필수)
cat > .openclaw.yml << 'EOF'
security:
  # 스킬 설치 비활성화 (CVE-2026-25253 방어)
  skills:
    enabled: false
...
### 보안
- [CVE-2026-25253 - NVD](https://nvd.nist.gov/)
