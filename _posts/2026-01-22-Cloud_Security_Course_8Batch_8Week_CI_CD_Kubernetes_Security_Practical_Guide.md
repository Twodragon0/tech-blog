...
redirect_from:
  ...
  - /posts/2026-01-22-Cloud_Security_Course_8Batch_8Week_CI_CD_Kubernetes_Security_Practical_Guide/
summary_card:
  ...
  tags:
    ...
    - "Pod-Security-Standards"
  highlights:
    - { source: "CI/CD 파이프라인 보안", title: "코드 스캔(SAST, DAST), 이미지 스캔(Trivy, Snyk), Secret 관리(HashiCorp Vault, AWS Secrets Manager), 파이프라인 무결성 검증(GPG 서명, 자동화된 테스트)" }
    - { source: "Kubernetes 네트워크 보안", title: "Network Policies(Pod 간 통신 제어), Service Mesh(Istio, Linkerd), Ingress Controller 보안, 네트워크 세분화 전략" }
    - { source: "Kubernetes 보안 아키텍처", title: "RBAC 최소 권한 원칙, Pod Security Standards(PSS), 감사 로깅(Kubernetes Audit), 모니터링 및 알림(Prometheus, Grafana)" }
    - { source: "AI 활용을 통한 DevSecOps 강화", title: "Cursor 보안 코딩, Claude API 자동화된 보안 검증, GitHub Copilot 보안 모범 사례, GitHub Actions AI 통합, AI 기반 보안 모니터링" }
    - { source: "DevSecOps 통합 전략", title: "자동화된 보안 검증(CI/CD 통합), 보안 강화된 파이프라인 구축, 정기적인 보안 감사, 지속적인 보안 모니터링" }
    - { source: "실전 보안 사례 및 실습", title: "보안 강화된 CI/CD 파이프라인 구축, Kubernetes 보안 환경 구성, 실전 보안 강화 사례" }
---
...
- [AI Coding Assistants Comparison](/posts/2026/01/17/AI_Coding_Assistants_Comparison_Gemini_Claude_Code_ChatGPT_OpenCode_2025_2026_Research_Analysis/)
...
- [8기 7주차: Docker/Kubernetes 보안 실습 가이드]({% post_url 2026-01-15-Cloud_Security_Course_8Batch_7Week_Docker_Kubernetes_Security_Practical_Guide %})
