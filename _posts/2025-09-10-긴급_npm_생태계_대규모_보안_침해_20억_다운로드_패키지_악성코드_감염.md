...
<img src="{{ '/assets/images/2025-09-10-npm_Large-scale_Security_20_image.jpg' | relative_url }}" alt="포스트 이미지" loading="lazy" class="post-image">
*그림: 포스트 이미지*
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│   1. 공격자: npmjs.help 피싱 사이트 구축                     │
│              │                                               │
│              ▼                                               │
│   2. Maintainer에게 "계정 보안 경고" 이메일 발송             │
│              │                                               │
│              ▼                                               │
│   3. 피해자가 가짜 사이트에서 로그인 시도                    │
│              │                                               │
│              ▼                                               │
│   4. 실시간으로 공식 npm에 자격증명 + 2FA 전달               │
│              │                                               │
│              ▼                                               │
│   5. 공격자 세션 탈취 → 패키지 악성 버전 배포                │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
1. **즉시 조치**
   - 모든 npm 프로젝트의 의존성 감사 실시
   - 영향받은 패키지 버전 즉시 업데이트
   - npm 2FA 설정 검토 및 하드웨어 키 사용 권장

2. **중기 조치**
   - SBOM(Software Bill of Materials) 도입 의무화
   - 의존성 고정(pinning) 정책 수립
   - 프라이빗 npm 레지스트리 미러링 고려
...
```bash
# GitLab Vulnerability Research Team 권장 도구

# 1. Socket.dev - 실시간 공급망 위협 탐지
npm install -g @socketsecurity/cli
socket scan

# 2. npm audit signatures - 패키지 서명 검증
...
- [GitLab Vulnerability Research Blog](https://about.gitlab.com/blog/categories/security/)
- [Socket.dev Security Reports](https://socket.dev/blog)
