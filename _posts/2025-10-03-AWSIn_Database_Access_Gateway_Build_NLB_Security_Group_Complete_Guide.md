...
{% include ai-summary-card.html
  ...
  highlights_html='<li><strong>포인트 1</strong>: ### 비즈니스 임팩트</li> <li><strong>포인트 2</strong>: 데이터베이스 접근 게이트웨이를 도입하면 다음과 같은 비즈니스 가치를 창출할 수 있습니다:</li> <li><strong>포인트 3</strong>: #### 보안 위험 감소 (80%)</li>'
...
> 
> ```hcl
> resource "aws_lb" "db_gateway" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_lb_target_group" "rds_mysql" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_security_group" "nlb" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_security_group" "database" { [truncated]
> ```
...
<!-- 전체 코드는 위 링크 참조
> **참고**: AWS WAF/CloudFront 설정 관련 내용은 [AWS WAF Terraform 모듈](https://github.com/trussworks/terraform-aws-wafv2) 및 [AWS WAF CloudFront 통합 예제](https://github.com/aws-samples/integrate-httpapi-with-cloudfront-and-waf)를 참조하세요. 연동
...
> 
> ```hcl
> resource "aws_wafv2_web_acl" "db_gateway" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_cloudwatch_dashboard" "db_gateway" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_cloudwatch_metric_alarm" "high_connection_count" { [truncated]
> ```
...
> 
> ```hcl
> resource "aws_sns_topic" "db_gateway_alerts" { [truncated]
> ```
...
> 
> ```python
> # lambda/slack_notifier.py [truncated]
> ```
