---
layout: post
title: "2026년 04월 03일 주간 보안 다이제스트: 제로데이·BYOVD EDR·DNS 유출 (15건)"
...
excerpt: "2026년 04월 03일 수집한 29건의 보안 이슈 중 해커들, CVE-2025-55182 악용해 766개 Next.js · Cisco, 원격 시스템 침해 가능한 9.8 CVSS IMC 및을 중심으로 영향 범위와 패치 우선순위를 분석합니다. 변경 통제와 모니터링 적용 시점, 사후 회고에 활용할 IoC 정리표를 포함합니다."
description: "Cisco FMC CVE-2025-55182(CVSS 9.8) 원격 시스템 침해 익스플로잇 분석, 에이전트 AI 시스템을 위한 네 가지 핵심 보안 원칙과 AWS 보안 업데이트를 중심으로 2026년 04월 03일 주요 보안·기술 뉴스 29건과 DevSecOps 대응 우선순위를 정리합니다."
...
| 분류 | 핵심 이슈 | 심각도 | 출처 |
|------|----------|--------|------|
| 🔒 **Security** | 해커들, CVE-2025-55182 악용해 766개 Next.js 호스트 침해 및 자격 증명 탈취 | 🟠 High | The Hacker News |
| 🔒 **Security** | Cisco, 원격 시스템 침해 가능한 9.8 CVSS IMC 및 SSM 취약점 패치 | 🔴 Critical | The Hacker News |
| 🔒 **Security** | 에이전트 AI 시스템을 위한 네 가지 보안 원칙 | 🟡 Medium | AWS Security Blog |
| 🤖 **AI/ML** | KernelEvolve: Meta의 랭킹 엔지니어 에이전트가 AI 인프라를 최적화하는 방법 | 🟡 Medium | Meta Engineering Blog |
| 🤖 **AI/ML** | RTX에서 Spark까지: NVIDIA, 로컬 에이전트 AI를 위한 Gemma 4 가속화 | 🟡 Medium | NVIDIA AI Blog |
| 🤖 **AI/ML** | Gemini API에서 비용과 신뢰성의 균형을 맞추는 새로운 방법 | 🟡 Medium | Google AI Blog |
| ☁️ **Cloud** | Honeylove, BigQuery로 제품 품질과 서비스 효율성 향상시키는 방법 | 🟡 Medium | Google Cloud Blog |
| ☁️ **Cloud** | Google Cloud에 Gemma 4 도입: 역대 가장 강력한 오픈 모델 | 🟡 Medium | Google Cloud Blog |
| ☁️ **Cloud** | vSphere와 BRICKSTORM 멀웨어: 방어자 가이드 | 🟡 Medium | Google Cloud Blog |
| ⚙️ **DevOps** | 소프트웨어 공급망을 방어하는 방법: 모든 엔지니어링 팀이 지금 해야 할 일 | 🟡 Medium | Docker Blog |

---

## 경영진 브리핑

- **긴급 대응 필요**: Cisco, 원격 시스템 침해 가능한 9.8 CVSS IMC 및 SSM 취약점 패치 등 Critical 등급 위협 1건이 확인되었습니다.
- **주요 모니터링 대상**: 해커들, CVE-2025-55182 악용해 766개 Next.js 호스트 침해 및 자격 증명 탈취, ThreatsDay Bulletin에서 확인된 사전 인증 체인 및 CloudTrail 회피 기법, 중국 연계 위협 그룹의 공급망 타깃 공격 동향 등 High 등급 위협 3건에 대한 탐지 강화가 필요합니다.
...
#### 1. 기술적 배경 및 위협 분석
이 공격은 Next.js 애플리케이션의 **React Server Components 및 App Router**에서 발생하는 **React2Shell 취약점(CVE-2025-55182)**을 초기 침투 벡터로 활용합니다. 이 취약점은 CVSS 10.0의 **원격 코드 실행(RCE)** 결함으로, 공격자가 인증 없이 임의의 쉘 명령을 실행할 수 있게 합니다. 공격자는 이를 통해 피해 호스트의 민감한 파일(DB 자격 증명, SSH 개인 키, AWS 시크릿, Stripe API 키, GitHub 토큰 등)을 체계적으로 탐색 및 탈취하는 **자격 증명 수확(Credential Harvesting)** 작업을 수행합니다. 766개 호스트가 영향을 받은 점으로 미루어, 공격자가 취약한 Next.js 인스턴스를 대규모로 스캔하고 자동화 공격을 수행한 것으로 보입니다.

아래는 스캔부터 자격 증명 탈취 및 횡적 이동까지 이어지는 공격 체인을 도식화한 것입니다.

```mermaid
sequenceDiagram
    participant A as 공격자
    participant N as Next.js 호스트 (RSC/App Router)
    participant F as 파일시스템/환경변수
    participant C as 클라우드·SaaS (AWS/GitHub/Stripe)
    A->>N: 1. 인터넷 대상 대규모 스캔 (취약 인스턴스 탐지)
    A->>N: 2. CVE-2025-55182 악용 (인증 없는 RCE)
    N->>F: 3. 민감 파일 탐색 (.env, id_rsa, 시크릿)
    F-->>N: 4. DB 자격증명·AWS 시크릿·토큰 반환
    N-->>A: 5. 수집된 자격 증명 유출 (Credential Harvesting)
    A->>C: 6. 탈취 토큰으로 횡적 이동·추가 인프라 침해
```
...
{% include news-card.html
  ...
  summary="Google이 Gemini API에 Flex와 Priority 두 가지 추론 서비스 티어를 도입했습니다. Flex 티어는 지연에 관대한 워크로드를 표준 대비 약 50% 저렴한 비용으로 처리하고, Priority 티어는 피크 시간대 우선 처리를 제공하며 필요 시 표준 티어로 폴백합니다. service_tier 파라미터로 비용과 신뢰성의 균형을 조절할 수 있습니다."
...
Flex Inference와 Priority Inference는 Gemini API의 비용과 신뢰성을 조정할 수 있는 두 가지 서비스 티어입니다. 개발자는 `service_tier` 파라미터로 지연을 허용하는 대신 표준가 대비 약 50% 저렴한 Flex와, 피크 시간대에 우선 처리되고 용량 초과 시 자동으로 Standard로 폴백하는 Priority 중에서 선택할 수 있습니다. 이를 통해 예산 제약 내에서도 워크로드 특성에 맞는 성능과 비용 효율을 달성할 수 있습니다.
...
| [나는 숫자가 아니다 – 72,000명 이상의 팔레스타인 희생자를 추모하며](https://news.hada.io/topic?id=28145) | GeekNews (긱뉴스) | 가자 지구 학살로 희생된 72,000명 이상의 팔레스타인인 을 추모하는 웹 기반 인터랙티브 추모 페이지 화면에는 60,199개의 빛 이 표시되어, 각 빛이 한 사람의 생명을 상징함 사용자는 마우스를 올려 개별 생명을 기억(remember) |
...
| Docker Blog | [docker.com](https://www.docker.com) | 본문 3건 인용 |
