...
- 의존성을 버전 태그가 아닌 다이제스트·커밋 SHA로 핀닝하고, 신규 릴리스에는 쿨다운(예: 3일)을 적용해 침해된 버전의 즉시 유입 차단
- SBOM 생성과 빌드타임 증명(attestation)을 CI에 통합하고 Docker Scout 등으로 알려진 취약점·악성 패키지를 지속 스캔
- axios 등 핵심 라이브러리 유지관리자 계정 탈취 사례에 대비해 의존성 변경 알림 및 lockfile 무결성 검증 강화
