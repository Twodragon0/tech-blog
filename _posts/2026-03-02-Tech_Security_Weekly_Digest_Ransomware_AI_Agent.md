...
{%- include news-card.html
  ...
  summary="Trump Media & Technology Group(DJT)이 소셜 미디어 플랫폼 Truth Social의 분사(Spin-out)를 검토 중이라고 밝혔다. 배경에는 그룹 차원의 **암호화폐·핀테크 사업 확대 전략**이 있다. 현재 Trump Media는 &quot;Truth."
...
{%- include news-card.html
  ...
  summary="X(구 트위터)가 **유료 파트너십 콘텐츠에 &quot;Paid Partnership&quot; 라벨**을 의무적으로 표시하는 정책을 도입한다. 주목할 점은 암호화폐 관련 프로모션을 유료 파트너십 프레임워크 하에서 허용하되, 별도 규정을 적용한다는 것이다. 이는 Elon Musk 인수 이후 전면 금지되었던 암호화폐 광고 정책의 사실상 부분 완화다."
  source="Cointelegraph"
-%}
...
{%- include news-card.html
  ...
  summary="미국 CFTC 승인을 받은 예측 시장 플랫폼 **Kalshi**의 창업자 Tarek Mansour가 이란 최고지도자 하메네이 관련 예측 시장에 대해 &quot;carve-out(예외 적용)&quot; 방침을 발표했다. Kalshi는 &quot;이 특정 마켓은 플랫폼에서 운영하지 않겠다&quot;는 입장을 명확히 했다."
  source="Cointelegraph"
-%}
...
{%- include news-card.html
  ...
  summary="한 개발자가 **광고 수익으로 운영되는 무료 AI 채팅 서비스**의 미래를 풍자적으로 구현한 실시간 데모를 공개했다. 실제 작동하는 언어 모델에 다양한 광고 형식을 통합하여, AI 서비스의 지속 가능한 수익화 모델에 대한 질문을 던진다."
  source="GeekNews"
-%}
...
{%- include news-card.html
  ...
  summary="**Anthropic**이 개발자를 위한 무료 온라인 교육 과정 **&quot;Anthropic Courses&quot;**를 공개했다. Claude 기본 사용법부터 시작하여 API 심화 활용, Claude Code 개발 워크플로, MCP(Model Context Protocol) 서버 구축, Agent Skills 개발까지 실무 중심 커리큘럼으로 구성되어 있다."
  source="GeekNews"
-%}
