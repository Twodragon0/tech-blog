...
```splunk
| 의심스러운 PowerShell 실행 + 외부 C2 통신 탐지
index=endpoint sourcetype=WinEventLog EventCode=4688
  (CommandLine="*powershell*" AND (CommandLine="*-enc*" OR CommandLine="*-EncodedCommand*"))
| eval is_encoded=if(match(CommandLine,"(?i)-e(nc(odedcommand)?)?\\s"), "YES", "NO")
| where is_encoded="YES"
| join ParentProcessId [
...
| table _time, host, user, CommandLine, dest_ip
| sort -_time
```

```splunk
| 대량 파일 암호화 이벤트 탐지 (랜섬웨어 조기 경보)
index=endpoint sourcetype=WinEventLog EventCode=4663
| stats count as file_ops by host, user, _time span=1m
| where file_ops > 500
| eval alert_level=if(file_ops > 2000, "CRITICAL", "HIGH")
| table _time, host, user, file_ops, alert_level
```
...
[사용자/시스템 입력]
       ↓
[입력 검증 & 샌드박스] ← 프롬프트 인젝션 방어
       ↓
[에이전트 추론 (Nemotron LTM)]
       ↓
[Action Policy 검증] ← 허용된 도구 호출만 실행
       ↓
[Human-in-the-Loop 게이트] ← 고위험 작업은 사람 승인 필수
       ↓
[실행 + 감사 로그]
```
