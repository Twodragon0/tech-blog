...
```mermaid
flowchart TD
    IN["사용자/시스템 입력"] --> VAL["입력 검증 & 샌드박스<br/>프롬프트 인젝션 방어"]
    VAL --> LLM["에이전트 추론<br/>Nemotron LTM"]
    LLM --> POL["Action Policy 검증<br/>허용된 도구만 실행"]
    POL --> HITL["Human-in-the-Loop<br/>고위험 작업 승인"]
    HITL --> EXEC["실행 + 감사 로그"]
```
