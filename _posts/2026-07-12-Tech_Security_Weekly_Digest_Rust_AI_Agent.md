...
| 분류 | 핵심 이슈 | 심각도 | 출처 |
|------|----------|--------|------|
| 🔒 **Security** | 손상된 jscrambler 8.14.0 npm 릴리스, 설치 중 Rust Infostealer 유포 | 🟡 Medium | The Hacker News |
...
sequenceDiagram
    ...
    participant N as npm 레지스트리
    participant D as 개발자/CI 파이프라인
    participant M as Rust Infostealer
    participant C as C2 서버
    participant S as Socket (모니터링)

    A->>N: jscrambler 8.14.0 악성 릴리스 게시 (preinstall hook 포함)
    N->>D: npm install 시 패키지 다운로드
    D->>M: preinstall hook 자동 실행
    M->>D: 시스템 정보·클라우드 자격증명 등 수집
    M->>C: 수집한 정보 외부 유출
    S-->>N: 게시 6분 후 악성 릴리스 탐지 및 플래그
```
...
```mermaid
flowchart LR
    A1[중국 연계 위협 그룹] -->|웹 애플리케이션 취약점 악용| P[Balochistan Police 포털 서버]
    A2[인도 연계 위협 그룹] -->|웹 애플리케이션 취약점 악용| P
    P -->|C2/백도어 설치| Persist["장기 침투 유지 (2024.02~2026.04)"]
    Persist --> D[범죄자·시민 데이터 유출]
```
...
해당 취약점은 Zimbra Classic Web Client에 존재하는 **저장형 XSS(Stored XSS)** 취약점으로, 특수하게 조작된 이메일이 사용자 세션에서 악성 스크립트를 실행할 수 있게 합니다. 아직 CVE는 할당되지 않았으며 공개된 CVSS 점수도 없지만, 본 다이제스트에서는 **Critical 등급**으로 분류해 긴급 패치를 권고합니다.
