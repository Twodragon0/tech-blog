...
redirect_from:
  ...
  - /posts/2026-01-15-Cloud_Security_Course_8Batch_7Week_Docker_Kubernetes_Security_Practical_Guide/
summary_card:
  ...
  tags:
    ...
    - "User-Namespaces"
  highlights:
    - { source: "컨테이너 보안 Best Practices", title: "이미지 스캔(Trivy, Snyk), Secret 관리(Kubernetes Secrets, External Secrets Operator), 비루트 사용자 실행, 읽기 전용 파일시스템, 최소 권한 원칙" }
    - { source: "Kubernetes 보안 아키텍처", title: "Pod Security Standards(PSS), User Namespaces(Kubernetes 1.33+), Network Policies, RBAC 최소 권한, Bound Service Account Tokens" }
    - { source: "Kubernetes 보안 Best Practices", title: "이미지 서명 및 검증(Cosign, Docker Content Trust), 런타임 보안 모니터링(Falco, Sysdig), 자동화된 보안 검증(CI/CD 통합), 정기적인 보안 감사" }
    - { source: "최신 보안 기능 (2024-2026)", title: "Kubernetes 1.32-1.35 보안 강화(User Namespaces Beta-by-Default, mTLS Pod Certificates), Kubernetes 1.36+ 예상 기능, Minikube 1.37.0+ 기능, K9s 보안 모범 사례" }
    - { source: "Docker/Container/Kubernetes 기본 이해", title: "Docker 이미지/컨테이너 개념, VM vs Container 비교, Kubernetes 핵심 리소스(Pod, Deployment, Service, Namespace), 컨테이너 격리 원리" }
---
