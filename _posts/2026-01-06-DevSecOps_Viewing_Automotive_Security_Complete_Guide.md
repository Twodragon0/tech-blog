...
excerpt: "DevSecOps 자동차 보안 완벽 가이드. SDV 보안 아키텍처, 자동차 위협 분석(네트워크/물리적/소프트웨어/공급망), DevSecOps 라이프사이클 통합, ISO 21434·UN R155/R156 규제 컴플라이언스, SAST/DAST/SBOM 도구 스택까지 실무 정리."
...
summary_card:
  ...
  tags:
    ...
    - "Supply-Chain-Security"
  highlights:
    - { source: "SDV(Software Defined Vehicle) 시대 보안", title: "중앙 집중식 컴퓨팅 플랫폼, 서비스 지향 아키텍처(SOA), OTA 업데이트 의존성, Zero Trust 내부 통신, HSM 통합, Secure Boot 체인" }
    - { source: "자동차 보안 위협 분석", title: "네트워크 공격(Wi-Fi/Bluetooth/4G/5G, V2X, OTA), 물리적 공격(OBD-II, ECU 탬퍼링), 소프트웨어 취약점(메모리/암호화/인증), 공급망 공격(악성 코드 삽입, 의존성 취약점)" }
    - { source: "DevSecOps 라이프사이클 통합", title: "Plan(위협 모델링 STRIDE/TARA), Code(SAST, Secret 스캔), Build(SCA, 이미지 스캔), Test(DAST/IAST, Fuzz), Release(SBOM, 서명/암호화), Deploy(Secure Boot), Monitor(런타임 보안, 침입 탐지)" }
    - { source: "규제 및 컴플라이언스", title: "ISO 21434(자동차 사이버 보안), UN R155/R156(2025년 글로벌 확대 적용), V2X 보안 표준화(IEEE 1609.2, SCMS), GDPR/개인정보보호법" }
    - { source: "실무 보안 도구 스택", title: "SAST(SonarQube, Semgrep, Clang Static Analyzer), Secret 스캔(Gitleaks, GitGuardian), SCA(Trivy, Snyk, Grype), DAST(OWASP ZAP, Burp Suite), Fuzz(AFL, LibFuzzer), SBOM(Syft, SPDX), 런타임 보안(Falco, Sysdig)" }
---
...
### 3.2 자동차 소프트웨어 개발 라이프사이클에 보안 통합
...
### 5.3 정적 및 동적 보안 테스트 적용

정적 애플리케이션 보안 테스트(SAST)와 동적 애플리케이션 보안 테스트(DAST)를 통해 코드의 보안 결함을 확인하고 시스템 내 침입을 시뮬레이션합니다.

#### 통합 보안 테스트 파이프라인

> 참고: 자동차 보안 스캔 관련 내용은 [GitHub Actions 보안 가이드](https://docs.github.com/en/actions) 및 [SonarQube](https://github.com/SonarSource/sonarqube)를 참조하세요.

![Mermaid Diagram](/assets/images/mermaid/DevSecOps_Viewing_Automotive_Security_Complete_Guide-mermaid-1.svg)

| 단계 | 보안 테스트 | 도구 | 목적 |
|------|-----------|------|------|
| SAST | 정적 분석 | SonarQube, Semgrep | 소스 코드 취약점 탐지 |
| Secret Scan | 시크릿 탐지 | Gitleaks, ggshield | 하드코딩된 자격 증명 탐지 |
| SCA | 의존성 분석 | Trivy, Snyk | CRITICAL/HIGH CVE 탐지 |
| SBOM | 자재 명세서 | Syft | SPDX JSON 형식 BOM 생성 |
| DAST | 동적 분석 | OWASP ZAP | 런타임 취약점 탐지 |
| Fuzz | 퍼징 테스트 | AFL | 예외 입력 처리 검증 |


## 6. 규정 준수: ISO 21434 및 UN R155

### 6.1 ISO 21434 (자동차 사이버 보안)
...
### 6.2 UN R155/R156 (자동차 사이버 보안 및 소프트웨어 업데이트 규정)
