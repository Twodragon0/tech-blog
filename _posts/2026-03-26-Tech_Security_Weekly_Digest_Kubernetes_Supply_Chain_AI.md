...
### 2.1 SLSA v1.1/SBOM 기반 공급망 보안 프레임워크 실무 적용
...
## 2. 실무 영향 분석
최근 Trivy, LiteLLM 등의 공급망 침해 사건을 고려할 때, SLSA 프레임워크의 도입은 선택이 아닌 필수가 되었습니다. 특히 CI/CD 파이프라인에서 빌드 아티팩트의 출처를 검증하지 않는 조직은 공급망 공격에 취약합니다. SBOM(Software Bill of Materials)과 결합하면 의존성 트리 전체의 무결성을 검증할 수 있습니다.
...
### 2.2 CVE 패치 관리 자동화: Dependabot과 Renovate 실전 전략
...
### 3.1 AI 에이전트 프롬프트 인젝션 공격의 신규 기법과 방어 전략
...
## 2. 실무 영향 분석
기업 내 AI 에이전트가 이메일 처리, 코드 리뷰, 문서 요약 등의 업무를 자동화하는 환경에서, 간접 프롬프트 인젝션은 **데이터 유출, 권한 남용, 악성 코드 실행**으로 이어질 수 있습니다. 특히 MCP(Model Context Protocol) 기반으로 외부 도구와 연동된 에이전트는 공격 표면이 넓어 더욱 주의가 필요합니다.

## 3. 대응 체크리스트
- [ ] **입력 검증 계층 구현**: 외부 데이터가 AI 에이전트에 전달되기 전에 악성 프롬프트 패턴을 탐지하는 필터링 계층을 구현합니다.
- [ ] **권한 최소화 원칙 적용**: AI 에이전트에게 부여하는 도구 접근 권한을 최소화하고, 위험한 작업(파일 삭제, 코드 실행)은 별도 승인 단계를 추가합니다.
- [ ] **출력 검증 파이프라인 구축**: AI 에이전트의 출력을 별도의 검증 모델이나 규칙 기반 필터로 검토한 후 실행합니다.
- [ ] **샌드박스 실행 환경 구성**: AI 에이전트의 도구 실행을 격리된 샌드박스 환경에서 수행하여 피해 범위를 제한합니다.
- [ ] **모니터링 및 이상 탐지**: AI 에이전트의 행동 패턴을 기록하고, 비정상적인 도구 호출이나 데이터 접근을 실시간으로 탐지합니다.
...
### 3.2 MCP(Model Context Protocol) 보안 가이드라인
...
### 4.1 Tekton Pipelines와 SLSA 통합 CI/CD 보안
...
### 4.2 ArgoCD + Kyverno 기반 GitOps 보안 정책 자동화
...
### 5.1 GKE Autopilot 보안 강화와 Workload Identity Federation
