...
redirect_from:
  ...
  - /posts/2026-02-01-Agentic_AI_Security_2026_Attack_Vectors_Defense_Architecture/
summary_card:
  ...
  tags:
    ...
    - "2026"
  highlights:
    - { source: "CrowdStrike", title: "AI Tool Poisoning - 도구 설명에 숨겨진 악성 지시로 AI 에이전트 조작" }
    - { source: "CrowdStrike", title: "Agentic Tool Chain Attack - AI 에이전트 공급망 공격의 새로운 벡터" }
    - { source: "Google", title: "Chrome 에이전틱 보안 아키텍처 - 샌드박스 기반 에이전트 격리" }
    - { source: "Google", title: "Prompt Injection 다층 방어 전략 - 입력 필터링부터 출력 검증까지" }
    - { source: "SK쉴더스", title: "LLM Application 취약점 진단 가이드 - 실무 점검 체크리스트" }
---
...
OWASP의 에이전틱 AI 보안 프레임워크를 함께 참고하세요.

- [OWASP 2025 최신 업데이트 완전 가이드: Top 10 에이전틱 AI 보안]({% post_url 2026-01-03-OWASP_2025_Latest_Update_Complete_Guide_Top_10_Agentic_AI_Security %})
...
- **자율형 AI 에이전트의 보안 패러다임 전환**: 단순 대화형 LLM에서 환경과 상호작용하고 코드를 자율 실행하는 Agentic AI로 진화함에 따라, 프롬프트 인젝션이 시스템 장악 및 원격 코드 실행(RCE)으로 직결되는 위험이 현실화되었습니다.
- **간접 프롬프트 인젝션(Indirect Prompt Injection) 방어**: 웹 검색 결과, 외부 문서, 이메일 등 에이전트가 인입하는 비신뢰 데이터에 숨겨진 악의적 지시사항을 무력화하기 위해 데이터 평면과 제어 평면의 엄격한 분리가 필수적입니다.
- **다계층 샌드박스 및 실시간 가드레일 확립**: 에이전트의 도구 호출을 Docker/gVisor 마이크로 VM 격리 환경으로 제한하고, 모델 입력 및 출력 단계에서 독립된 보안 모델이 개입하는 이중 검증 구조를 수립해야 합니다.
