...
반면 [Claude Code](https://docs.anthropic.com/en/docs/claude-code/overview)는 Anthropic이 공식 개발한 CLI 기반 에이전틱 코딩 도구입니다. 터미널에서 직접 코드를 읽고, 편집하고, 테스트를 실행하며, Git 워크플로우까지 처리하는 전문 개발 도구입니다.
...
2026년 1월에 공개된 [CVE-2026-25253](https://nvd.nist.gov/vuln/detail/CVE-2026-25253)은 AI의 스킬(Skill) 시스템에 존재하는 원격 코드 실행(RCE) 취약점입니다. CVSS 점수 8.8로 High Severity에 해당합니다.
...
3. 스킬의 `on_install` 훅이 실행되면서 임의 코드가 사용자 시스템에서 실행됩니다.
...
## 📋 엔터프라이즈 AI 코딩 도구 도입 평가 체크리스트 (AI Evaluation Checklist)

- [ ] **컨텍스트 윈도우 및 대규모 코드베이스 이해도**: 200k~1M 이상의 롱 컨텍스트를 활용해 멀티 모듈 리포지토리의 파일 간 의존성을 정확히 파악하고 코드를 생성하는지 확인합니다.
- [ ] **로컬 터미널 및 빌드/테스트 도구 연동성**: 컴파일, 단위 테스트(`pytest`, `npm test`), 린트(`ruff`, `eslint`)를 에이전트 루프 내에서 자체 실행하고 디버깅할 수 있는지 점검합니다.
- [ ] **보안 샌드박스 및 파일 시스템 쓰기 통제**: 개발자 머신의 중요 디렉토리 접근을 통제하고 Git 커밋 전 변경 사항을 명시적으로 검토할 수 있는 안전장치가 있는지 검증합니다.
- [ ] **토큰 비용 최적화 및 캐싱(Prompt Caching) 지원**: 대화 기록 증가에 따른 API 비용 폭증을 방지하기 위해 컨텍스트 캐싱(Context Caching) 메커니즘을 지원하는지 확인합니다.
- [ ] **팀 단위 지식 공유 및 커스텀 프롬프트/룰 통합**: `.claude/rules`나 `.cursorrules`처럼 프로젝트 표준 코딩 컨벤션을 일관되게 주입할 수 있는 설정 파일이 지원되는지 점검합니다.
