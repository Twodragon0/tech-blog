...
excerpt: "2억 8500만 달러 규모 Drift 해킹 · 신종 피싱 문자에 QR 코드를 활용한 교통위반 사기 등장 등 2026년 04월 06일 보고된 15건의 보안/기술 이슈를 운영 관점에서 점검합니다. 실무 체크리스트에 패치 적용 항목을 함께 정리했습니다."
...
- **주요 모니터링 대상**: 2억 8500만 달러 규모 Drift 해킹(6개월 북한 사회공학 작전), 악성 npm 패키지 36개의 Redis·PostgreSQL 백도어 배포 등 High 등급 위협 2건에 대한 탐지 강화가 필요합니다.
...
| 제목 | 출처 | 핵심 내용 |
|------|------|----------|
| [Awesome Design.MD - 유명 웹사이트 디자인 시스템을 내 사이트에 적용하기](https://news.hada.io/topic?id=28246) | GeekNews (긱뉴스) | DESIGN.md 는 Google Stitch가 도입한 개념 으로, AI 에이전트가 읽고 일관된 UI를 생성하기 위한 텍스트 디자인 문서 Figma, JSON 스키마, 별도 툴링 없이 마크다운 파일 하나 만 프로젝트 |
...
이번 주기의 핵심 트렌드는 **피싱 & 사회공학**(2건)입니다. 2억 8500만 달러 규모 Drift 해킹이 6개월간의 북한 사회공학 작전으로 밝혀졌으며, **취약점** 측면에서는 FortiClient EMS 긴급 패치가 필요합니다. QR 코드 피싱 등 새로운 사회공학 기법 확산에도 주의가 요구됩니다.
...
오늘의 두 핵심은 **FortiClient EMS CVE-2026-35616(in-the-wild 악용, 주말 긴급 패치)** 과 **Solana 기반 Drift DEX 의 2억 8,500만 달러 피해(6개월간 DPRK 사회공학 캠페인의 결과)** 다. 둘은 코드 결함과 인적 공격이라는 다른 결처럼 보이지만, 같은 답을 강요한다 — **신뢰 경계 안쪽의 "관리 인프라" 와 "관리자 권한 보유자" 가 가장 비싼 자산**이라는 점이다. EMS 한 대가 뚫리면 그 안에서 관리되는 모든 FortiClient 엔드포인트로 악성 정책을 푸시할 수 있고, 한 명의 관리자 키가 사회공학으로 넘어가면 6개월간 쌓아 올린 스마트컨트랙트 거버넌스가 한 트랜잭션으로 무력화된다.

이번 주 한 줄: FortiClient EMS 측은 노출 점검부터 시작하라 — `nmap -p 8013,10443 --script http-title -T4 <EMS-CIDR>` 으로 관리 콘솔 응답을 확인한 뒤, 8013/10443 이 사내망 외부 또는 사용자 VLAN 에서 도달 가능하면 즉시 ACL 격리 + 패치 적용 P0. Drift 측 교훈은 운영 키 관리로 옮겨야 한다 — 사내 IAM/Privileged Access 환경에서 `aws iam list-access-keys --user-name <admin> --query 'AccessKeyMetadata[?CreateDate<=`'$(date -v-90d +%F)'`].[UserName,AccessKeyId,CreateDate]'` (또는 `date -d "-90 days"` GNU 변형) 으로 90일 넘긴 관리자 액세스 키를 색출하고, 90일 이내 강제 회전·MFA 강제 정책을 SCP 로 박아두라.

본 블로그의 [AWS 클라우드 보안 완전 가이드 — IAM·EKS·보안 아키텍처](https://tech.2twodragon.com/posts/2026/01/14/AWS_Cloud_Security_Complete_Guide_IAMFrom_EKSTo_Security_Architecture/) 에서 정리한 권한 분리(SoD)·JIT·HSM 기반 키 관리 패턴은 Drift 케이스의 6개월 사회공학 시나리오가 왜 한 명의 키 보유자에서 끊겼어야 했는지를 직접 보여준다. 다중서명·하드웨어 격리가 도입된 환경이라면 같은 사회공학으로 한 명을 뚫어도 자산 이동은 막힌다 — 이번 주 작업할 항목은 그 다중서명 경계의 현행 상태 검증이다.
