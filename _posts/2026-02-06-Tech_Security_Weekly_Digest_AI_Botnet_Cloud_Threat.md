...
excerpt: "2026-02-06 기술·보안 주간 다이제스트: CrashFix · 실무 관점에서 영향 범위와 우선순위를 함께 점검해야 합니다를 비롯한 2026년 02월 06일 보안/기술 동향 15건을 DevSecOps 시선으로 정리합니다. 영향받는 자산 식별과 SBOM 기반 의존성 패치, EDR 룰 보강 가이드를 다룹니다."
...
{%- include news-card.html
  title="[AI/ML] Claude Opus 4.6 - Vertex AI 출시와 AI 에이전트 보안"
  url="https://cloud.google.com/blog/products/ai-machine-learning/expanding-vertex-ai-with-claude-opus-4-6/"
  image="https://storage.googleapis.com/gweb-cloudblog-publish/images/021326b_HF1428_GC_Social_ClaudeSonnet_4.6_.max-2500x2500.jpg"
  summary="Google Cloud에서 Anthropic의 최신 모델 Claude Opus 4.6을 Vertex AI에서 사용할 수 있게 되었다고 발표했습니다. Claude Opus 4.6는 복잡한 코딩 작업과 고도화된 AI 에이전트 생성에서 뛰어난 성능을 보이며, 기존 모델 대비 더 깊은 맥락 파악 능력과 향상된 지시 따르기 성능을 제공합니다."
...
{%- include news-card.html
  title="[클라우드] Dragonfly v2.4.0 출시 - P2P 기반 컨테이너 이미지 배포"
  url="https://www.cncf.io/blog/2026/02/05/dragonfly-v2-4-0-is-released/"
  image="https://www.cncf.io/wp-content/uploads/2026/01/Akamia-Cloud-Credits-28.png"
  summary="CNCF 프로젝트 Dragonfly v2.4.0이 출시되었습니다. P2P(Peer-to-Peer) 기반 컨테이너 이미지 배포 시스템으로, 대규모 클러스터에서 이미지 풀(pull) 시간을 획기적으로 단축합니다. 이번 버전에서는 부하 인식 스케줄링 알고리즘(중앙 스케줄링 + 노드 수준 보조 스케줄링 2단계)이 추가되었습니다."
  source="CNCF Blog"
  severity="Medium"
-%}
...
| 레이어 | 구성 요소 | 주요 설정 |
|--------|-----------|-----------|
| L7 Application | AWS WAF + CloudFront | Rate-based rules, Geo-blocking, Managed rule groups, Bot signature matching |
| L4 Load Balancing | ALB / NLB | Connection draining, Cross-zone load balancing, Health checks |
| Auto Scaling | EC2 / ECS | Min:2 / Desired:4 / Max:20(Cost Cap), DDoS Mode 시 Max:8 |
| Origin | Protected Servers | CDN 뒤에 Origin IP 비노출 필수 |
