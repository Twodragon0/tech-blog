...
{%- include news-card.html
  ...
  summary="Google Cloud에서 Anthropic의 최신 모델 **Claude Opus 4.6**을 Vertex AI에서 사용할 수 있게 되었다고 발표했습니다. Claude Opus 4.6는 복잡한 코딩 작업과 고도화된 AI 에이전트 생성에서 뛰어난 성능을 보이며, 기존 모델 대비 더 깊은 맥락 파악 능력과 향상된 지시 따르기 성능을 제공합니다."
...
{%- include news-card.html
  ...
  summary="CNCF 프로젝트 **Dragonfly v2.4.0**이 출시되었습니다. P2P(Peer-to-Peer) 기반 컨테이너 이미지 배포 시스템으로, 대규모 클러스터에서 이미지 풀(pull) 시간을 획기적으로 단축합니다. 이번 버전에서는 **부하 인식 스케줄링 알고리즘**(중앙 스케줄링 + 노드 수준 보조 스케줄링 2단계)이 추가되었습니다."
  source="CNCF Blog"
-%}
