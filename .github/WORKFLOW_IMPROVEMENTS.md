# 워크플로우 개선사항

## 🔍 발견된 문제

### 1. DeepSeek API 타임아웃 오류

**문제:**
- `HTTPSConnectionPool(host='api.deepseek.com', port=443): Read timed out.`
- 긴 포스트(10,000자 이상) 처리 시 30초 타임아웃으로 인한 실패

**원인:**
- 타임아웃 시간이 너무 짧음 (30초)
- 재시도 로직 없음
- 네트워크 지연 대응 부족

## ✅ 적용된 개선사항

### 1. 타임아웃 시간 증가
- **변경 전**: 30초
- **변경 후**: 120초
- 긴 포스트 처리 시 충분한 시간 확보

### 2. 재시도 로직 추가
- **최대 재시도 횟수**: 3회
- **재시도 전략**: 지수 백오프 (Exponential Backoff)
  - 1차 재시도: 2초 대기
  - 2차 재시도: 4초 대기
  - 3차 재시도: 8초 대기

### 3. 에러 핸들링 개선
- 타임아웃 오류와 일반 오류 구분
- 각 시도마다 상세한 로그 기록
- 최종 실패 시 명확한 오류 메시지

### 4. 로깅 개선
- 재시도 시도 횟수 표시
- 대기 시간 표시
- 오류 유형별 상세 메시지

## 📝 코드 변경 사항

### `scripts/generate_audio.py`

```python
# 재시도 로직 추가
max_retries = 3
retry_delay = 2  # 초기 재시도 대기 시간 (초)

for attempt in range(1, max_retries + 1):
    try:
        # 지수 백오프 적용
        if attempt > 1:
            wait_time = retry_delay * (2 ** (attempt - 2))
            time.sleep(wait_time)
        
        # 타임아웃 증가
        timeout_seconds = 120  # 30초 → 120초
        
        response = requests.post(
            DEEPSEEK_API_URL,
            json=data,
            headers=headers,
            timeout=timeout_seconds
        )
        
        # 성공 시 처리...
        
    except requests.exceptions.Timeout as e:
        # 타임아웃 오류 처리
        if attempt < max_retries:
            continue
        return None
    except requests.exceptions.RequestException as e:
        # 일반 네트워크 오류 처리
        if attempt < max_retries:
            continue
        return None
```

## 🎯 예상 효과

1. **안정성 향상**: 재시도 로직으로 일시적 네트워크 오류 극복
2. **성공률 증가**: 타임아웃 시간 증가로 긴 포스트 처리 가능
3. **사용자 경험 개선**: 명확한 오류 메시지와 재시도 진행 상황 표시

## 🔄 추가 개선 제안

### 1. 요청 크기 최적화
- 매우 긴 포스트는 청크로 나누어 처리
- 또는 포스트 요약 후 처리

### 2. 캐싱 추가
- 동일 포스트 재처리 시 캐시된 대본 사용
- 비용 절감 및 속도 향상

### 3. 모니터링 강화
- API 응답 시간 추적
- 실패율 모니터링
- 알림 설정

### 4. 비동기 처리
- 여러 포스트 병렬 처리
- 처리 시간 단축

## 📊 성능 지표

### 개선 전
- 타임아웃 오류 발생률: 높음
- 긴 포스트 처리 성공률: 낮음
- 평균 처리 시간: 불안정

### 개선 후 (예상)
- 타임아웃 오류 발생률: 낮음 (재시도로 대부분 해결)
- 긴 포스트 처리 성공률: 높음
- 평균 처리 시간: 안정적

## 🔗 관련 문서

- [Troubleshooting Video Gen](./TROUBLESHOOTING_VIDEO_GEN.md)
- [Cost Management](./COST_MANAGEMENT.md)
- [Workflow Test Guide](./WORKFLOW_TEST_GUIDE.md)
