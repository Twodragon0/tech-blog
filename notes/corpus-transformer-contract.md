...
| 변환기 | C11 | 비고 |
|--------|-----|------|
| 마침표 `backfill_card_summary_period` | ✅ | `news_card_patterns` import + 재현율 테스트 |
| 되감기 `rewind_truncated_summaries` | ✅ | 동일 |
| 구조 복원 `restore_digest_structure` | ❌ | 대상이 카드가 아니라 헤딩/마커 — **자기 도메인의 재현율 테스트가 따로 필요** |
| 참고자료 `enrich_digest_references` | ❌ | 대상이 `## 참고 자료` 섹션 — 동일 |
| 링크화 `linkify_bare_urls` | ❌ | 대상이 맨 URL — 동일 |

카드 계열 2종은 `scripts/news_card_patterns.py` 를 단일 진실 원천으로 쓰고,
`scripts/tests/test_news_card_patterns.py` 가 **`매칭 수 == 코퍼스 실제 인스턴스 수`** 를
파일 단위로 고정한다. ground truth는 그 모듈의 정규식이 아니라 **리터럴 문자열 `.count()`**
로 독립 산출한다(C5). 정규식을 `\{%\s*include` 로 되돌리면 7건이 실패한다(실측).

나머지 3종은 "카드"가 대상이 아니므로 이 모듈을 쓸 수 없다. **각자의 대상 정의에 대해
같은 형태의 재현율 테스트를 따로 만들어야 한다** — C11은 "카드 정규식을 공유하라"가
아니라 "대상을 다 찾았음을 테스트로 고정하라"는 계약이다.

**#523이 실제로 드러낸 것:** 정규식만 고쳐도 코퍼스는 0건 변경이었다. 사각지대의
진짜 결함 9건은 `rewind_truncated_summaries.py` 의 `_TERMINATED = (".", "!", "?")` 가
`...` 로 끝나는 요약을 "완성 문장"으로 판정해 건너뛴 것이었다. 그 `...` 는 200자 캡
절단 마커이므로 종결의 반대 증거다. **재현율 결함과 판정 결함이 겹쳐야 드러나는 층**이
있으니, C11 테스트를 통과했다고 대상 처리가 옳다는 뜻은 아니다.
