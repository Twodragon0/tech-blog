...
**Status:** ✅ **완료 (2026-08-05)** — digest 코퍼스 고유명사 위반 **0건**. 설계 확정 2026-07-28.
...
3. **Phase 2b — 구조-defective 98개: ▶ 진행 중 (블로커 해소됨, 2026-07-31).**
   ...
   - ✅ **완료 (2026-08-05)**: 2월 파일럿(02-17) · 3월 파일럿(03-15) · **4월 23개** ·
     **5월 27개** · **6월 28개** · **최종 19개**(7월 10 · 3월 8 · 2월 1).
     코퍼스 98 → 75 → 74 → 47 → 19 → **0**.
...
### Phase 2b 파티션 파일럿 진행 (per-post 스테이지드)

파티션별로 proper-noun 결함 파일을 열거하고 **구조 상태로 분류**한다: 구조-clean이면 즉시
`--fix`(+ 필요 시 체크리스트 헤딩 canonical화), 구조-defective면 편집 캠페인으로 defer.

- **2월 파티션 (2026-07-30, PR #477):** 33개 중 proper-noun 결함 **2개**(02-08, 02-17), 둘 다
  구조결함 동반. **02-17 = 클린**(`## 조치 체크리스트`→`## 실무 체크리스트` 리네임 + --fix) 완료.
  **02-08 = defer**(`## 1.` 헤더 부재·경영진브리핑 깨짐·넘버링 체크리스트 — 이질 구조).
- **3월 파티션 (2026-07-30, PR #480):** 29개 중 proper-noun 결함 **9개**, **전부 구조-defective**.
  심각도 3-tier:
  - 경미(체크리스트 이름/번호): 03-11·03-12·03-15. **03-15만 클린 단일수정**(`## 6. 실무 체크리스트`가
    마지막 번호 섹션 → 번호 제거로 canonical, 넘버링 [1-5] 유지) 완료. 03-11·03-12는 체크리스트가
    중간 섹션이라 **후속 재넘버링 필요** → defer.
  - 중간(per-item 대응 체크리스트 헤딩): 03-17·03-18 → defer.
  - 심각(body H1 + 중복 넘버링 `[1,1,2,3,…]` + per-item 체크리스트, 구형 per-item-분석 템플릿):
    03-20·03-21·03-24·03-25 → defer.
  - **결론:** 3월은 2월보다 훨씬 defective(클린 1/9). "전용 편집 캠페인" 권고를 강하게 재확인.
    윈도우 DEFER도 실증(03-15 "Parallels 윈도우 VM" 올바르게 불변).
- **5월 파티션 (2026-08-04, PR #492):** 30개 중 proper-noun 결함 **27개**, **전건 canonical화 완료**
  (168건 치환: 비트코인 126 / 리눅스 13 / 이더리움 12 / 쿠버네티스 7 / 안드로이드·구글 각 3 /
  마이크로소프트 2 / 깃허브·시스코 각 1). 구조 상태와 무관하게 처리 — 4월과 동일 절차.
  변경 155줄 전부 allow-list 치환만으로 정확히 재구성됨(**오변환 0**). 인용 제목 `"…친비트코인
  성향…"`은 word-boundary로 정상 불변, 05-21의 `윈도우`도 DEFER대로 불변.
  - **래칫 오탐 1건 발견·수정:** 05-05가 "+1 new"로 차단됐는데 결함 집합은 6→6 동일. 원인은
    위반 메시지가 **본문 텍스트를 embed**(`body H1 heading found: # … 안드로이드 …`)해 래칫의
    문자열 multiset 비교가 content-sensitive했던 것. `new_violations`를 **kind 단위** 비교로
    수정(`_kind()`). 같은 종류의 *추가* 인스턴스(H1 2→3)는 계속 검출, *리워딩*만 면제.
- **6월 파티션 (2026-08-04, PR #493):** 30개 중 결함 **28개**, 전건 canonical화 완료
  (198건: 비트코인 156 / 쿠버네티스 9 / 리눅스·이더리움 각 8 / **메타 6** / 안드로이드 4 /
  마이크로소프트·도커·아마존 각 2 / 시스코 1). 변경 175줄 전부 allow-list 치환만으로
  재구성 — **오변환 0**. 래칫 신규 위반 0(156 grandfathered) → #492의 kind-단위 비교가
  28개 규모에서도 유효함을 확인.
  - **메타 6건 사전 문맥 검증:** 전부 회사 Meta(크리에이터 스테이블코인 지급 06-07,
    타사 비즈니스 데이터 활용 06-10). `메타데이터`/`메타버스`류 접두사 용례는 0건이라
    #481의 하이픈 deny가 발동할 케이스는 없었다. 아마존=Amazon(CEO Andy Jassy),
    도커=Docker(컨테이너 빌드), 시스코=Cisco(Unified CM CVE-2026-20230)도 개별 확인.
- **최종 파티션 (2026-08-05, PR #495):** 잔여 19개(7월 10 · 3월 8 · 2월 1) 전건 canonical화
  → **코퍼스 FAIL 0 달성**. 107건 치환(비트코인 76 / **메타 15** / 이더리움·마이크로소프트·
  안드로이드·구글 각 3 / 텔레그램 2 / 쿠버네티스·리눅스 각 1). 변경 95줄 전부 allow-list
  치환만으로 재구성 — **오변환 0**. 07-30의 `메타-하네스` 2건은 `_ENTITY_DENY`대로 불변.
  - **`메타` 공백 접두사 변종 발견 → deny 확장.** 하이픈 deny(#481)만으로는 부족하다:
    공백도 `[^가-힣]` 분기를 만족하므로 `메타 주제`("meta-level theme")가 통과해
    `Meta 주제`로 훼손된다. `_posts/` 전수 실측 결과 `메타+구분자`는 4건 —
    SPACE+`광고의` ×3(07-16, **진짜 Meta**), SPACE+`주제입니다` ×1(02-02), HYPHEN+`하네스` ×2.
    `_ENTITY_DENY["메타"]`를 `[-–—]|\s*주제`로 확장(좁은 enumeration — blanket "메타+공백"
    규칙은 `메타 광고` 3건의 정당 치환을 놓친다).
  - **증거 강도 주의:** `2026-02-02-Weekly_Tech_AI_Blockchain_Digest.md`는 파일명에
    `Weekly_Digest`가 없어 **`_is_digest_post` 스코프 밖**이다. 즉 `메타 주제`는 가드의
    실측 FP가 *아니고*, 저자가 그 표현을 쓴다는 증거에 근거한 **예방적** deny다
    (blogwatcher가 발행 시 `--fix`를 자동 실행하므로 크론 digest에 나오면 본문이 훼손된다).
- **⚠ 위 2·3월 defer 판정은 래칫 도입(2026-07-31) *전* 기록이다.** "구조-defective →
  proper-noun 백필 defer"라는 전제는 위 "구조 게이트 diff 래칫" 절에서 무효화됐고,
  **4월 파티션 23개(전부 구조-defective)가 반증**했다. 3월 잔여 8개도 구조 백필 선행 없이
  4월과 동일 절차로 재개할 수 있다. 다만 03-15처럼 구조를 *함께* 정리한 건은 그대로 유효하다.
