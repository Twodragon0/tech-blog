...
| `tests/visual-baselines/` + `scripts/svg_visual_baseline.py` (pixel-diff, ≤0.5 % px and ≤100 px contiguous block) | `visual-baseline-refresh.yml` (`--capture`, push to main — **auto-commits**) | `visual-baseline-verify.yml` (`--verify`, PR only, wired 2026-08-10). Both jobs pin `ubuntu-22.04`; mixing runner versions between capture and verify yields false positives. `TARGET_SVGS` is a hardcoded 30-cover sample, so this is a canary, not corpus coverage. |

Consequences to keep in mind: adding a `schedule` to `visual-regression.yml` buys nothing (it would re-run the same fixed date set daily). The pixel-diff half was capture-only until 2026-08-10 — and because refresh **auto-commits** on main, a generator change that altered these renders would have been recorded as the new truth with nothing having objected first; `visual-baseline-verify.yml` now runs `--verify` on PRs, the one point where both the old and new renders exist. `tests/visual-baselines/` also holds 27 orphan PNGs (7.3 MB) left over from an earlier `TARGET_SVGS` list — unreferenced by `manifest.json`, so `--verify` ignores them. `--verify` also cannot be validated locally on macOS — baselines are captured on `ubuntu-22.04`, and a local run shows a uniform ~1.5 % diff with an identical `max_block=1134px` across all 30 files, i.e. a platform rendering delta rather than drift.
