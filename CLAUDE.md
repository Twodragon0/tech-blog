...
│   ├── lazy_data_generator.rb  # Emits /tags-data.json, /categories-data.json, /archive-data.json at :site,:post_write
│   ├── svg_to_png_og.rb        # Rewrites post.image .svg → _og.png at :pre_render
│   └── markdown_image_lazy.rb  # Inject loading="lazy" on body <img>
├── tags-data.json       # (REMOVED — now generated by _plugins/lazy_data_generator.rb)
├── categories-data.json # (REMOVED — same as above)
├── .claude/
│   ├── settings.json    # Project-scoped Claude Code config (hooks wired here)
│   ├── settings.local.json  # Per-developer overrides (gitignored)
│   ├── agents/          # Sub-agents (post-researcher, security-auditor, ...)
│   ├── commands/        # Custom skills (/new-post, /validate-post, ...)
│   ├── hooks/           # Pre/PostToolUse hooks (validate-svg, post-edit-check, canonical-url-gate)
│   └── rules/           # Coding/security/perf/git rules loaded by agents
...
| `tests/visual-baselines/` + `scripts/svg_visual_baseline.py` (pixel-diff, ≤0.5 % px and ≤100 px contiguous block) | `visual-baseline-refresh.yml` (`--capture`, push to main — **auto-commits**) | `visual-baseline-verify.yml` (`--verify`, PR only, wired 2026-08-10). Both jobs pin `ubuntu-22.04`; mixing runner versions between capture and verify yields false positives. `TARGET_SVGS` is a hardcoded 32-cover sample (28 L20 + 2 unattributed + 2 rollup), so this is a canary, not corpus coverage — and the verify trigger lists only the generators that sample can actually detect. |

Consequences to keep in mind: adding a `schedule` to `visual-regression.yml` buys nothing (it would re-run the same fixed date set daily). The pixel-diff half was capture-only until 2026-08-10 — and because refresh **auto-commits** on main, a generator change that altered these renders would have been recorded as the new truth with nothing having objected first; `visual-baseline-verify.yml` now runs `--verify` on PRs, the one point where both the old and new renders exist. The 27 orphan PNGs (7.3 MB) left over from an earlier `TARGET_SVGS` list were removed on 2026-08-11 — they were unreferenced by `manifest.json`, so nothing read them. `--verify` cannot be validated locally on macOS — baselines are captured on `ubuntu-22.04`, and a local run shows a uniform ~1.5 % diff with an identical `max_block=1134px` across all 30 files, i.e. a platform rendering delta rather than drift.
...
- [ ] `sitemap: false` (boolean) — NOT `sitemap.exclude: yes` (jekyll-sitemap ignores nested form; custom sitemap.xml now handles both, but stick to boolean)
- [ ] Canonical/share URLs use `{{ site.url }}{{ page.url }}` — NEVER `absolute_url` (prepends site.baseurl, breaks GH Pages backup canonicals — see `notes/seo-canonical-fix.md`)
- [ ] `lsp_diagnostics` clean on changed files
- [ ] Template branch changes: `pytest scripts/tests/` passes (pre-commit hook auto-runs)
- [ ] GH Pages workflow path filter — add new top-level files (`.github/workflows/deploy-pages.yml`) when creating root-level `.html`, `.json`, or `.xml` artifacts so the backup deploys them.

### SEO Anti-Patterns (banned — auto-flagged by `.claude/hooks/canonical-url-gate.sh`)

In `_includes/head.html`, `sitemap.xml`, `feed.xml`, `llms*.txt`:

| Don't | Do |
|-------|-----|
| `{{ page.url \| absolute_url }}` | `{{ site.url }}{{ page.url }}` |
| `{{ site.url }}{{ site.baseurl }}{{ post.url }}` | `{{ site.url }}{{ post.url }}` |
| `{{ og_image \| absolute_url }}` | `{{ site.url }}{{ og_image }}` |

`absolute_url` adds `site.baseurl`. On the GH Pages backup the build runs with
`--baseurl "/tech-blog"`, so it produces `https://tech.2twodragon.com/tech-blog/...`
URLs that 404 on Vercel. Google sees the broken canonical and refuses to
index the page. Asset paths (favicon, embedded `<img>` src, RSS feed link)
**keep** `relative_url` because they should resolve to whichever origin
serves the HTML.
